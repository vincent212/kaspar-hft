...
#ifndef _SBE_MDINCREMENTALREFRESHVOLUMELONGQTY66_H_
#define _SBE_MDINCREMENTALREFRESHVOLUMELONGQTY66_H_
...
class MDIncrementalRefreshVolumeLongQty66
{
private:
...
public:
    ...
    static const std::uint16_t SBE_TEMPLATE_ID = static_cast<std::uint16_t>(66);
    ...
    MDIncrementalRefreshVolumeLongQty66() = default;
    ...
    MDIncrementalRefreshVolumeLongQty66(char *buffer, const std::uint64_t bufferLength) :
        MDIncrementalRefreshVolumeLongQty66(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    MDIncrementalRefreshVolumeLongQty66(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        MDIncrementalRefreshVolumeLongQty66(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    {
        return static_cast<std::uint16_t>(66);
    }
    ...
    MDIncrementalRefreshVolumeLongQty66 &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        return *this = MDIncrementalRefreshVolumeLongQty66(buffer, offset, bufferLength, sbeBlockLength(), sbeSchemaVersion());
    }

    MDIncrementalRefreshVolumeLongQty66 &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        ...
        return *this = MDIncrementalRefreshVolumeLongQty66(
            buffer,
    ...
    MDIncrementalRefreshVolumeLongQty66 &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
        const std::uint64_t bufferLength)
    {
        return *this = MDIncrementalRefreshVolumeLongQty66(buffer, offset, bufferLength, actingBlockLength, actingVersion);
    }

    MDIncrementalRefreshVolumeLongQty66 &sbeRewind()
    {
    ...
    {
        MDIncrementalRefreshVolumeLongQty66 skipper(m_buffer, m_offset, m_bufferLength, sbeBlockLength(), m_actingVersion);
        skipper.skip();
...
#pragma clang diagnostic pop
    ...
    MDIncrementalRefreshVolumeLongQty66 &transactTime(const std::uint64_t value) SBE_NOEXCEPT
    {
...
template<typename CharT, typename Traits>
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, MDIncrementalRefreshVolumeLongQty66 _writer)
{
    MDIncrementalRefreshVolumeLongQty66 writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "MDIncrementalRefreshVolumeLongQty66", )";
