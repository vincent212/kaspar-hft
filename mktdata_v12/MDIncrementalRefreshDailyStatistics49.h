...
#ifndef _SBE_MDINCREMENTALREFRESHDAILYSTATISTICS49_H_
#define _SBE_MDINCREMENTALREFRESHDAILYSTATISTICS49_H_
...
public:
    ...
    static const std::uint16_t SBE_TEMPLATE_ID = static_cast<std::uint16_t>(49);
    ...
    MDIncrementalRefreshDailyStatistics49() = default;
    ...
    MDIncrementalRefreshDailyStatistics49(char *buffer, const std::uint64_t bufferLength) :
        MDIncrementalRefreshDailyStatistics49(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    MDIncrementalRefreshDailyStatistics49(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        MDIncrementalRefreshDailyStatistics49(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    {
        return static_cast<std::uint16_t>(49);
    }
    ...
    MDIncrementalRefreshDailyStatistics49 &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        return *this = MDIncrementalRefreshDailyStatistics49(buffer, offset, bufferLength, sbeBlockLength(), sbeSchemaVersion());
    }

    MDIncrementalRefreshDailyStatistics49 &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        ...
        return *this = MDIncrementalRefreshDailyStatistics49(
            buffer,
    ...
    MDIncrementalRefreshDailyStatistics49 &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
        const std::uint64_t bufferLength)
    {
        return *this = MDIncrementalRefreshDailyStatistics49(buffer, offset, bufferLength, actingBlockLength, actingVersion);
    }

    MDIncrementalRefreshDailyStatistics49 &sbeRewind()
    {
    ...
    {
        MDIncrementalRefreshDailyStatistics49 skipper(m_buffer, m_offset, m_bufferLength, sbeBlockLength(), m_actingVersion);
        skipper.skip();
...
#pragma clang diagnostic pop
    ...
    MDIncrementalRefreshDailyStatistics49 &transactTime(const std::uint64_t value) SBE_NOEXCEPT
    {
...
public:
    ...
    #pragma clang diagnostic pop
        ...
        SBE_NODISCARD MDEntryTypeDailyStatistics::Value mDEntryType() const
        {
            char val;
            std::memcpy(&val, m_buffer + m_offset + 24, sizeof(char));
            return MDEntryTypeDailyStatistics::get((val));
        }

        NoMDEntries &mDEntryType(const MDEntryTypeDailyStatistics::Value value) SBE_NOEXCEPT
        {
            char val = (value);
...
template<typename CharT, typename Traits>
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, MDIncrementalRefreshDailyStatistics49 _writer)
{
    MDIncrementalRefreshDailyStatistics49 writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "MDIncrementalRefreshDailyStatistics49", )";
