...
#ifndef _SBE_ADMINLOGIN15_H_
#define _SBE_ADMINLOGIN15_H_
...
public:
    static const std::uint16_t SBE_BLOCK_LENGTH = static_cast<std::uint16_t>(1);
    ...
    AdminLogin15(char *buffer, const std::uint64_t bufferLength) :
        AdminLogin15(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    AdminLogin15(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        AdminLogin15(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    {
        return *this = AdminLogin15(buffer, offset, bufferLength, sbeBlockLength(), sbeSchemaVersion());
    }

    AdminLogin15 &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    AdminLogin15 &wrapForDecode(
        ...
        const std::uint64_t bufferLength)
    {
        return *this = AdminLogin15(buffer, offset, bufferLength, actingBlockLength, actingVersion);
    }

    AdminLogin15 &sbeRewind()
    {
    ...
    {
        AdminLogin15 skipper(m_buffer, m_offset, m_bufferLength, sbeBlockLength(), m_actingVersion);
        skipper.skip();
...
#pragma clang diagnostic pop
    ...
    static SBE_CONSTEXPR std::int8_t heartBtIntNullValue() SBE_NOEXCEPT
    {
        return SBE_NULLVALUE_INT8;
    }

    static SBE_CONSTEXPR std::int8_t heartBtIntMinValue() SBE_NOEXCEPT
    {
        return static_cast<std::int8_t>(-127);
    }

    static SBE_CONSTEXPR std::int8_t heartBtIntMaxValue() SBE_NOEXCEPT
    {
        return static_cast<std::int8_t>(127);
    }
    ...
    SBE_NODISCARD std::int8_t heartBtInt() const SBE_NOEXCEPT
    {
        std::int8_t val;
    ...
    AdminLogin15 &heartBtInt(const std::int8_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, AdminLogin15 _writer)
{
    AdminLogin15 writer(
        _writer.m_buffer,
        _writer.m_offset,
