...
#ifndef _SBE_GROUPSIZE8BYTE_H_
#define _SBE_GROUPSIZE8BYTE_H_
...
public:
    ...
    GroupSize8Byte(
        ...
        const std::uint64_t bufferLength,
        const std::uint64_t actingVersion) :
        GroupSize8Byte(buffer, 0, bufferLength, actingVersion)
    {
    }
    ...
    GroupSize8Byte(
        char *buffer,
        const std::uint64_t bufferLength) :
        GroupSize8Byte(buffer, 0, bufferLength, sbeSchemaVersion())
    {
    }
    ...
    GroupSize8Byte &wrap(
        ...
        const std::uint64_t bufferLength)
    {
        return *this = GroupSize8Byte(buffer, offset, bufferLength, actingVersion);
    }
...
#pragma clang diagnostic pop
    ...
    GroupSize8Byte &blockLength(const std::uint16_t value) SBE_NOEXCEPT
    {
...
#pragma clang diagnostic pop
    ...
    GroupSize8Byte &numInGroup(const std::uint8_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, GroupSize8Byte writer)
{
    builder << '{';
