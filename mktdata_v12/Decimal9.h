...
#ifndef _SBE_DECIMAL9_H_
#define _SBE_DECIMAL9_H_
...
public:
    ...
    Decimal9(
        ...
        const std::uint64_t bufferLength,
        const std::uint64_t actingVersion) :
        Decimal9(buffer, 0, bufferLength, actingVersion)
    {
    }
    ...
    Decimal9(
        char *buffer,
        const std::uint64_t bufferLength) :
        Decimal9(buffer, 0, bufferLength, sbeSchemaVersion())
    {
    }
    ...
    Decimal9 &wrap(
        ...
        const std::uint64_t bufferLength)
    {
        return *this = Decimal9(buffer, offset, bufferLength, actingVersion);
    }
...
#pragma clang diagnostic pop
    ...
    Decimal9 &mantissa(const std::int64_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, Decimal9 writer)
{
    builder << '{';
    builder << R"("mantissa": )";
