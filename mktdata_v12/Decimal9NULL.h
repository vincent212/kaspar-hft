...
#ifndef _SBE_DECIMAL9NULL_H_
#define _SBE_DECIMAL9NULL_H_
...
public:
    ...
    Decimal9NULL(
        ...
        const std::uint64_t bufferLength,
        const std::uint64_t actingVersion) :
        Decimal9NULL(buffer, 0, bufferLength, actingVersion)
    {
    }
    ...
    Decimal9NULL(
        char *buffer,
        const std::uint64_t bufferLength) :
        Decimal9NULL(buffer, 0, bufferLength, sbeSchemaVersion())
    {
    }
    ...
    Decimal9NULL &wrap(
        ...
        const std::uint64_t bufferLength)
    {
        return *this = Decimal9NULL(buffer, offset, bufferLength, actingVersion);
    }
...
#pragma clang diagnostic pop
    ...
    Decimal9NULL &mantissa(const std::int64_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, Decimal9NULL writer)
{
    builder << '{';
    builder << R"("mantissa": )";
