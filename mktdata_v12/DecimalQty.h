...
#ifndef _SBE_DECIMALQTY_H_
#define _SBE_DECIMALQTY_H_
...
public:
    ...
    DecimalQty(
        ...
        const std::uint64_t bufferLength,
        const std::uint64_t actingVersion) :
        DecimalQty(buffer, 0, bufferLength, actingVersion)
    {
    }
    ...
    DecimalQty(
        char *buffer,
        const std::uint64_t bufferLength) :
        DecimalQty(buffer, 0, bufferLength, sbeSchemaVersion())
    {
    }
    ...
    DecimalQty &wrap(
        ...
        const std::uint64_t bufferLength)
    {
        return *this = DecimalQty(buffer, offset, bufferLength, actingVersion);
    }
...
#pragma clang diagnostic pop
    ...
    DecimalQty &mantissa(const std::int32_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, DecimalQty writer)
{
    builder << '{';
    builder << R"("mantissa": )";
