...
which a predictive placement model can be scored; the reported costs come from a
reference implementation that wraps the model-free core in state-reading safety
guards, and separating the two is left to future work. The algorithm and the order-book
...
\begin{enumerate}[leftmargin=1.6em]
  ...
  passive-order placement --- with the qualification, carried through the paper,
  that every number reported here comes from the reference implementation's
  guarded superset rather than from the model-free core in isolation
  (Section~\ref{sec:prod}) --- built on \emph{participant shadowing}: on observing a
...
One qualification bounds that claim. The costs of Section~\ref{sec:eval} were
produced by the reference implementation, which wraps the model-free core in the
state-reading guards of Section~\ref{sec:prod} --- a two-band placement
hysteresis and the completion and over-hedge limits. Those guards consult the
inside price and recent trade sizes, so the measured figures are not, strictly,
the cost of a model-free method; they are the cost of a method whose
\emph{placement and cancellation} decisions are model-free and whose safety
layer is not. Ablating the guards and re-running the core alone is the
experiment that would settle how much the safety layer contributes, and it is
