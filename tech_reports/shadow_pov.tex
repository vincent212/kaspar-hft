...
replication.
Both rows cover the full corpus: 246 sessions, and $1.6$ million individual
passive fills. Intervals are 95\,\% and clustered by session.}
\label{tab:markout}
\footnotesize
...
We ran the passive configuration of Table~\ref{tab:styles} --- shadowing
$5\,\%$ of adds within a $3$-tick band --- twice over the same 246 sessions,
once with zero delay and once with the order, the cancel and the feed each
delayed by thirty seconds. The algorithm, the price source, the band, the rate
and the corpus are identical. The only difference is how long after observing an
order the shadow reaches the exchange.

\begin{table}[!htbp]
\centering
\caption{The same passive configuration run against itself, with and without a
thirty-second delay on the order, the cancel and the feed. ES futures, 246
sessions of calendar 2025, 100 contracts bought and 100 sold per window, $5\,\%$
of adds shadowed within a $3$-tick band. Thirty seconds is chosen as a control
...
The comparison is between the method and itself, which is what makes it hard to
argue with: there is no alternative strategy whose fairness can be disputed, and
no parameter differs between the two rows but one. What it establishes is that
