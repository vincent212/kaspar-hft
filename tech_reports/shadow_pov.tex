...
prints; the experiments use $2\%$) and, when selected, places a marketable order
...
In the reference implementation both arms are in fact the \emph{same} C++ class
(\texttt{light22}): the aggressive behaviour is selected by configuration --- a
non-zero per-trade placement rate --- rather than written as a separate
strategy, so the passive and aggressive versions share their placement,
cancellation, and safety-guard code and are implemented side by side.

Two mechanical details distinguish it from the passive arm. First, the
...
silently become a passive order the arm never asked for. Because the resting
order a trade references is being consumed at the moment the shadow attaches to
it, the identifier-driven cancel of Section~\ref{sec:cancel} fires immediately
rather than after a grace window. Second, the aggressive arm runs in its
\emph{own} bank --- one light per side, with its own queue coordinator and a
shared position coordinator --- because a marketable order clears its slot on
arrival rather than resting, so a single light sustains the target rate without
crowding itself out. It uses the wider placement/cancel bands noted above
($10$ and $12$ ticks) appropriate to an order that expects to cross. Everything
else --- the codebase, the corpus, the position mandate, and the completion and
over-hedge guards --- is identical to the passive arm, so the two differ only in
which market event they follow and in whether they rest or cross. This is the
configuration reported as ``Aggressive POV'' throughout Section~\ref{sec:eval}.
...
\bibitem{scheduletactics2014} V.~Markov, S.~Mazur, and D.~Saltz.
  ...
  The Journal of Trading, Fall 2011. arXiv:1409.1441.
\bibitem{limitmarkettactics2014} V.~Markov.
  \emph{On the Design of Sell-Side Limit and
  Market Order Tactics.}
  The Journal of Trading, 7(3):29--39, 2012. arXiv:1409.1442.
