...
\paragraph{6. Taken to thirty seconds, the information is worth $0.27$ ticks per
contract.} The millisecond range measures a slope. To measure the whole of what
the coupling is worth, we ran the passive configuration of
Table~\ref{tab:styles} --- shadowing $5\,\%$ of adds within a $3$-tick band ---
twice over the same 246 sessions, once with zero delay and once with the order,
the cancel and the feed each delayed by \emph{thirty seconds}. Everything else
is identical: the same algorithm, the same price source, the same band, the same
rate, the same corpus. The only difference is how long after observing an order
the shadow reaches the exchange.

\begin{center}
\small
\begin{tabular}{lrrrr}
\toprule
{\color{black}Delay} & Windows & Rel.\ slippage & Participation & Time \\
\midrule
None        & 9{,}554 & $+0.0953 \pm 0.0130$ & 4.47\,\% & 72\,s \\
30 seconds  & 9{,}594 & $+0.3673 \pm 0.0989$ & 1.56\,\% & 44\,s \\
\bottomrule
\end{tabular}
\end{center}

Cost rises by a factor of $3.9$, on intervals that do not approach each other,
and three further columns move with it: participation falls to a third,
the parent is worked in $37$ child fills per leg rather than $85$, and the legs
finish \emph{sooner} --- because a quote priced thirty seconds ago arrives
marketable and crosses on contact rather than resting, which is item~2 at an
extreme. The difference, $0.272$ ticks per contract, is what acting immediately
on an observed order is worth over acting on the same observation half a minute
later.

The comparison is between the method and itself, which is what makes it hard to
argue with: there is no alternative strategy to dispute the fairness of. What it
establishes is that the inherited information is real and that it is
\emph{perishable}. It does not establish what that information is worth against
placing with none at all --- at thirty seconds the price is still inherited from
a real participant's order, merely stale --- and a comparison against a
placement rule coupled to nothing remains the experiment of
Section~\ref{sec:future}.
