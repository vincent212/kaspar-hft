...
The placement decision is thus model-free --- price and venue are read off the
observed order --- which is what lets it serve as a benchmark for passive-order
placement on central limit order book (CLOB) venues.
Model-free is not information-free. \pov{} consumes a great deal of market
information: it reads every order-book message and places only where a
participant has just committed capital. The distinction is that it
\emph{computes} nothing from that information: the price it rests at is copied
from an observed order, and the decision to withdraw is a lookup on that order's
identifier. Information is inherited from the flow rather than derived from a
model, and the distinction matters for what the benchmark measures.
...
We propose \pov{} as a model-free benchmark for passive-order placement, against
which a predictive placement model can be scored once the comparisons of
Section~\ref{sec:future} have been run. The algorithm and the order-book
...
The residual gap does not carry the result. The same sweep gives the cost at
several other participation rates, and the cost does not move with the rate: the
aggressive style runs $+0.0884 \pm 0.0257$ at $1.05\,\%$ participation,
$+0.0726 \pm 0.0194$ at $1.90\,\%$, $+0.0869 \pm 0.0171$ at $3.18\,\%$,
$+0.0952 \pm 0.0135$ at $4.86\,\%$ and $+0.0870 \pm 0.0101$ at $6.85\,\%$ ---
a $6.5$-fold range of participation, every interval overlapping every other, and
no trend in the rate. A gap of four tenths of a percentage point between the two
styles therefore cannot be what produces their agreement. This is the behavior
the simulator's zero-impact assumption predicts, and finding it is one check
that the assumption is being applied consistently: with no impact model there is
no channel through which taking a larger share of the volume can cost more, and
none appears.
