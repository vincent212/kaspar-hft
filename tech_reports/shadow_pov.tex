...
% DRAFT MARKING. A diagonal watermark on every page plus a footer line, because
% a watermark alone is easy to miss on a printed page and a footer alone is easy
% to crop. Both say the same thing. Remove this block, not the text under it,
% when the paper is ready to circulate.
...
\fancyhf{}
\fancyfoot[C]{\thepage}
\fancyfoot[L]{\footnotesize\textbf{DRAFT --- NOT FOR DISTRIBUTION}}
...
Every figure in this paper rests on one inference, which is worth stating on its
own. A shadow enters the price-time-priority (FIFO) queue at its price, behind
...
This is an inference from the recorded feed, not an observation. The synthetic
order was never in the book and nothing in the capture reports its fate. It is
sound under price-time priority and exact queue accounting, both of which the
reconstruction provides; it is not sound if the shadow's own presence would have
changed the flow.

\paragraph{What the simulator does not provide.} Market reaction. Orders are
...
the only basis on which their costs can be set side by side.

\begin{table}[t]
\centering
\caption{Passive and aggressive execution at matched participation. ES futures,
...
quantity, computed from different data at different grains, and they agree. Passive figures are 20 sessions and aggressive 246;
...
every other parameter fixed within each style.}
\label{tab:latency}
\small
...
aggressive equivalent at matched participation, and swept the simulated latency
...
$9{,}592$ completed windows a round trip of 100 contracts bought and 100 sold
costs $+0.0955 \pm 0.0130$ ticks per contract against the arrival mid --- under
