...
participant has just committed capital. What it never does is \emph{compute}
anything from that information --- no forecast, no fill probability, no
valuation of queue position. The information is inherited from the flow rather
than derived from a model, and the distinction matters for what the benchmark
measures.
...
conclusion. What \pov{} never does is \emph{compute}: it builds no model of the
book, estimates no fill probability, values no queue position, and forecasts no
price. It free-rides on the modelling other participants have already done
