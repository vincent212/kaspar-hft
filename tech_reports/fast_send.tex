...
onto a single thread via \fs{}, each hop costing $\sim\!36$~ns --- about
$62\times$ less --- so it incurs no queue and no context switch. That servicing
...
nothing is left. The \fs{} row is the point of the paper: at $\sim\!36$~ns it is
about $62\times$ cheaper than the cross-thread \snd{}, because it is, mechanically,
a locked function call.
...
amortized (see text). Ungrouped $p99$ is $6750$~ns; the grouped and \fs{} rows
...
--- macOS has a fast small-object allocator, so $124.5$~ns falls only to
$92.8$~ns, and this understates what a busier or Linux allocator would show. The
decisive effect is in the tail: in a separate run the \emph{maximum} round trip
...
from a \texttt{MemoryPool}, turning a $\sim\!15$~ns global allocation into a
$\sim\!2$~ns pooled one and removing the allocator tail. What these numbers do
...
depth; and they were taken on Apple Silicon, not the x86-64 Linux target. That
