...
\title{\textbf{Evaluating Suitability of the Actor Model of Concurrency for\\
...
from data races, resistance to lock-ordering deadlocks, and sequential
single-message reasoning --- have been recognized for decades, yet the model has
long been dismissed as unsuitable for high-frequency trading (HFT). The reasoning was that actors imply many
...
strongly clustered, consistent with a self-exciting process (under a Hawkes
interpretation the observed Fano scaling implies a branching ratio $\approx
0.85$--$0.97$). The latency the
...
execution under receiver transparency --- is the design point \fs{}
...
its per-message cost is the subject of this paper.
...
\cite{concur2026}. The actor discipline eliminates the hazard that
...
with \fs{} --- but a monitor is a passive object invoked by a direct method
call: it has no mailbox, offers no asynchronous alternative, and does not present
the callee as an isolated actor that cannot observe how it was reached.
...
onto a single thread via \fs{}, each hop costing $\sim\!10$~ns --- about one
virtual call, three orders of magnitude less than a thread-crossing \snd{} (see
Table~\ref{tab:transport}) --- so it incurs no queue and no context switch. That servicing
...
intervening queue and no context switch between stages. This is the chained
synchronous execution of Section~\ref{sec:mech}: an $N$-stage chain with
...
Section~\ref{sec:micro} --- is tens of nanoseconds: a \fs{} hop adds $\sim\!10$~ns
over a direct call (about one virtual call; the full ping/pong round trip is
...
$\text{idx}\times100$~ns, and $\text{idx}$ is largest in the tail. The design
lever is therefore the slope rather than the floor or the mailbox queue, which
...
A trading operation is judged on the tail, not the median.
...
$\sim\!10$~ns hop is below every figure in these tables.
...
The framework does not appear in this decomposition. The floor is decode and
book work, and the slope is protocol serialization; neither is the framework. The
actor abstraction --- the mailbox handoff and the \fs{}/\snd{} dispatch --- costs
about one virtual call per hop ($\sim\!10$~ns), a few tens of nanoseconds across
the collapsed decode/book/signal chain, against a $\sim\!7~\mu$s floor: well under
$1\%$, and it does not grow with in-packet position, so it makes no measurable
contribution to the tail under this workload.
...
a percent of the irreducible work and makes no measurable contribution to the
observed tail. What an
...
$99.3\%$ to $99.6\%$ of messages have another ahead of them; a trade message
almost always shares its packet, and the median ZN~trade message is ninth in its
packet, having incurred eight slopes before it is decoded.
...
races, resistance to lock-ordering deadlocks, and sequential single-message
reasoning. This paper supports
...
change in packet rate, and the framework's own per-hop overhead ($\sim\!10$~ns,
about one virtual call) is well under $1\%$ of it. The tail is not the framework and not, primarily, the queue: with in-packet
...
arrival process, which is non-Poisson and strongly clustered, consistent with a
self-exciting process (under a Hawkes interpretation the Fano scaling implies a
branching ratio $\approx 0.85$--$0.97$), and converts bursts into large packets
and in-packet
...
and to no measurable contribution to the tail, which belongs to the market's
arrival process and not to the messaging layer. An HFT data path built on this
model therefore pays essentially no latency penalty for the actor discipline, and
in return obtains data-race freedom, resistance to lock-ordering deadlocks,
single-message reasoning that is unusually
