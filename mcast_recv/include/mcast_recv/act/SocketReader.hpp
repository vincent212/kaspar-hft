...
namespace mcast_recv
{
  ...
  private:
    ...
    // because a stride counter here is the obvious thing to reach for again,
    // and the measurement that says not to is in read()'s comment.
    //
    // This drops #117, which made ts_cnt unsigned so `ts_cnt % 16` would lower
    // to a bitmask instead of a division sequence. That was a correct fix to
    // the modulo. It is dropped because the counter it optimised should not
    // exist: the subsample it drove was zeroing recv_ts on 15 of every 16
    // packets, and every one of those lands in rej_zero_t0.
