...
**Why actors?** Each actor owns its private state and communicates only by messages, so no mutable state is shared between actors — and therefore no memory-level data race, and no locks in your own code; you reason about one message at a time against consistent state. The whole class of multithreading bugs — data races, deadlocks, torn reads and writes, lock-ordering mistakes — goes away entirely: there is no shared mutable state and the framework owns all the concurrency, so there are simply no locks in your code to get wrong. Actor code is also unusually easy for AI coding agents to write. They understand the actor
