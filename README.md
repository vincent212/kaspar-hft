...
**Why actors?** Each actor owns its private state and communicates only by messages, so no mutable state is shared between actors — and therefore no memory-level data race, and no locks in your own code; you reason about one message at a time against consistent state. (Empirical studies call data races and deadlocks *"two mistakes that are hard to make with actors."* It is not a silver bullet — higher-level hazards remain, blocking or cyclic `fast_send` can still deadlock, and the framework synchronizes internally — but the everyday shared-memory data races are designed out of application code.) The usual objection is the messaging overhead; Kaspar answers it with `fast_send`, which runs the receiver's handler inline on the caller's thread and returns the reply as a value (~tens of ns round trip). The design and measurements are written up in [**tech_reports/fast_send.pdf**](tech_reports/fast_send.pdf) (benches in [`actors/cpp/perf`](actors/cpp/perf)).
