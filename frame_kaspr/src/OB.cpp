...
// Install the return-path publisher so fills and cancel-acks pay the same
// inbound latency as a book update. A fill is generated BY the record being
// processed, so it happened at that instant and reaches us feed_delay later.
// NOT the SOM Ack -- that is a local acceptance that never crossed a wire.
void act::OB::install_return_path()
{
  frame::ob::Order::delayed_publish =
    [this](actors::Actor *to, actors::Message *m) {
      publish_delayed(to, m, this->last_processed_ts);
    };
}
