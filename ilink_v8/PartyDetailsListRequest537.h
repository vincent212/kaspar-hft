...
#ifndef _SBE_PARTYDETAILSLISTREQUEST537_CXX_H_
#define _SBE_PARTYDETAILSLISTREQUEST537_CXX_H_
...
public:
    ...
    static const std::uint16_t SBE_TEMPLATE_ID = static_cast<std::uint16_t>(537);
    ...
    {
        return static_cast<std::uint16_t>(537);
    }
    ...
    PartyDetailsListRequest537 &sbeRewind()
    {
    ...
    {
        PartyDetailsListRequest537 skipper(m_buffer, m_offset, m_bufferLength, sbeBlockLength(), m_actingVersion);
        skipper.skip();
    ...
    #pragma GCC diagnostic pop
            ...
            m_bufferLength = bufferLength;
            GroupSize dimensions(buffer, *pos, bufferLength, actingVersion);
            dimensions.blockLength(static_cast<std::uint16_t>(7));
...
template<typename CharT, typename Traits>
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const PartyDetailsListRequest537 &_writer)
{
    PartyDetailsListRequest537 writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << ", ";
    {
        ...
        writer.noRequestingPartyIDs().forEach(
            [&](NoRequestingPartyIDs &noRequestingPartyIDs)
            {
                if (atLeastOne)
                {
                    builder << ", ";
...
void skip()
{
    auto &noRequestingPartyIDsGroup { noRequestingPartyIDs() };
    while (noRequestingPartyIDsGroup.hasNext())
    {
        noRequestingPartyIDsGroup.next().skip();
    }
    auto &noPartyIDsGroup { noPartyIDs() };
    while (noPartyIDsGroup.hasNext())
    {
        noPartyIDsGroup.next().skip();
    }
}
...
#pragma GCC diagnostic ignored "-Wtype-limits"
    ...
    if (noRequestingPartyIDsLength > 254LL)
    {
