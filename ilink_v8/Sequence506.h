...
#ifndef _SBE_SEQUENCE506_CXX_H_
#define _SBE_SEQUENCE506_CXX_H_
...
public:
    static const std::uint16_t SBE_BLOCK_LENGTH = static_cast<std::uint16_t>(14);
    static const std::uint16_t SBE_TEMPLATE_ID = static_cast<std::uint16_t>(506);
    ...
    Sequence506(char *buffer, const std::uint64_t bufferLength) :
        Sequence506(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    Sequence506(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        Sequence506(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    {
        return static_cast<std::uint16_t>(506);
    }
    ...
    Sequence506 &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        return *this = Sequence506(buffer, offset, bufferLength, sbeBlockLength(), sbeSchemaVersion());
    }

    Sequence506 &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    Sequence506 &wrapForDecode(
        ...
        const std::uint64_t bufferLength)
    {
        return *this = Sequence506(buffer, offset, bufferLength, actingBlockLength, actingVersion);
    }

    Sequence506 &sbeRewind()
    {
    ...
    {
        Sequence506 skipper(m_buffer, m_offset, m_bufferLength, sbeBlockLength(), m_actingVersion);
        skipper.skip();
    ...
    Sequence506 &uUID(const std::uint64_t value) SBE_NOEXCEPT
    {
    ...
    Sequence506 &nextSeqNo(const std::uint32_t value) SBE_NOEXCEPT
    {
    ...
    Sequence506 &faultToleranceIndicator(const FTI::Value value) SBE_NOEXCEPT
    {
    ...
    SBE_NODISCARD static const char *KeepAliveIntervalLapsedMetaAttribute(const MetaAttribute metaAttribute) SBE_NOEXCEPT
    {
        switch (metaAttribute)
        {
            case MetaAttribute::SEMANTIC_TYPE: return "int";
    ...
    static SBE_CONSTEXPR std::uint16_t keepAliveIntervalLapsedId() SBE_NOEXCEPT
    {
        return 39016;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::uint64_t keepAliveIntervalLapsedSinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }

    SBE_NODISCARD bool keepAliveIntervalLapsedInActingVersion() SBE_NOEXCEPT
    {
        return true;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::size_t keepAliveIntervalLapsedEncodingOffset() SBE_NOEXCEPT
    {
        return 13;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::size_t keepAliveIntervalLapsedEncodingLength() SBE_NOEXCEPT
    {
        return 1;
    }

    SBE_NODISCARD std::uint8_t keepAliveIntervalLapsedRaw() const SBE_NOEXCEPT
    {
        std::uint8_t val;
    ...
    SBE_NODISCARD KeepAliveLapsed::Value keepAliveIntervalLapsed() const
    {
        std::uint8_t val;
        std::memcpy(&val, m_buffer + m_offset + 13, sizeof(std::uint8_t));
        return KeepAliveLapsed::get((val));
    }

    Sequence506 &keepAliveIntervalLapsed(const KeepAliveLapsed::Value value) SBE_NOEXCEPT
    {
        std::uint8_t val = (value);
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const Sequence506 &_writer)
{
    Sequence506 writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << ", ";
    builder << R"("KeepAliveIntervalLapsed": )";
    builder << '"' << writer.keepAliveIntervalLapsed() << '"';
