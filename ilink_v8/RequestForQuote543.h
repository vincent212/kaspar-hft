...
#ifndef _SBE_REQUESTFORQUOTE543_CXX_H_
#define _SBE_REQUESTFORQUOTE543_CXX_H_
...
public:
    static const std::uint16_t SBE_BLOCK_LENGTH = static_cast<std::uint16_t>(55);
    static const std::uint16_t SBE_TEMPLATE_ID = static_cast<std::uint16_t>(543);
    ...
    RequestForQuote543(char *buffer, const std::uint64_t bufferLength) :
        RequestForQuote543(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    RequestForQuote543(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        RequestForQuote543(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    {
        return static_cast<std::uint16_t>(543);
    }
    ...
    RequestForQuote543 &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        return *this = RequestForQuote543(buffer, offset, bufferLength, sbeBlockLength(), sbeSchemaVersion());
    }

    RequestForQuote543 &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        ...
        return *this = RequestForQuote543(
            buffer,
    ...
    RequestForQuote543 &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
        const std::uint64_t bufferLength)
    {
        return *this = RequestForQuote543(buffer, offset, bufferLength, actingBlockLength, actingVersion);
    }

    RequestForQuote543 &sbeRewind()
    {
    ...
    {
        RequestForQuote543 skipper(m_buffer, m_offset, m_bufferLength, sbeBlockLength(), m_actingVersion);
        skipper.skip();
    ...
    RequestForQuote543 &partyDetailsListReqID(const std::uint64_t value) SBE_NOEXCEPT
    {
    ...
    RequestForQuote543 &quoteReqID(const std::uint64_t value) SBE_NOEXCEPT
    {
        std::uint64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    RequestForQuote543 &manualOrderIndicator(const ManualOrdIndReq::Value value) SBE_NOEXCEPT
    {
    ...
    RequestForQuote543 &seqNum(const std::uint32_t value) SBE_NOEXCEPT
    {
    ...
    RequestForQuote543 &senderID(const std::uint64_t index, const char value)
    {
        if (index >= 20)
        {
    ...
    RequestForQuote543 &putSenderID(const char *const src) SBE_NOEXCEPT
    {
    ...
    #if __cplusplus >= 201703L
    RequestForQuote543 &putSenderID(const std::string_view str)
    {
    ...
    #else
    RequestForQuote543 &putSenderID(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 20)
        {
    ...
    RequestForQuote543 &sendingTimeEpoch(const std::uint64_t value) SBE_NOEXCEPT
    {
        std::uint64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    RequestForQuote543 &location(const std::uint64_t index, const char value)
    {
        if (index >= 5)
        {
    ...
    RequestForQuote543 &putLocation(const char *const src) SBE_NOEXCEPT
    {
    ...
    #if __cplusplus >= 201703L
    RequestForQuote543 &putLocation(const std::string_view str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 5)
        {
    ...
    #else
    RequestForQuote543 &putLocation(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 5)
        {
    ...
    SBE_NODISCARD static SBE_CONSTEXPR std::size_t quoteTypeEncodingLength() SBE_NOEXCEPT
    {
        return 1;
    }

    SBE_NODISCARD std::uint8_t quoteTypeRaw() const SBE_NOEXCEPT
    {
        std::uint8_t val;
        std::memcpy(&val, m_buffer + m_offset + 54, sizeof(std::uint8_t));
        return (val);
    }

    SBE_NODISCARD QuoteTyp::Value quoteType() const
    {
        std::uint8_t val;
        std::memcpy(&val, m_buffer + m_offset + 54, sizeof(std::uint8_t));
        return QuoteTyp::get((val));
    }

    RequestForQuote543 &quoteType(const QuoteTyp::Value value) SBE_NOEXCEPT
    {
        std::uint8_t val = (value);
        std::memcpy(m_buffer + m_offset + 54, &val, sizeof(std::uint8_t));
        return *this;
    }
    ...
    #pragma GCC diagnostic pop
        ...
        NoRelatedSym &orderQty(const std::uint32_t value) SBE_NOEXCEPT
        {
            std::uint32_t val = SBE_LITTLE_ENDIAN_ENCODE_32(value);
        ...
        SBE_NODISCARD RFQSide::Value side() const
        {
            std::uint8_t val;
            std::memcpy(&val, m_buffer + m_offset + 8, sizeof(std::uint8_t));
            return RFQSide::get((val));
        }

        NoRelatedSym &side(const RFQSide::Value value) SBE_NOEXCEPT
        {
            std::uint8_t val = (value);
...
template<typename CharT, typename Traits>
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const RequestForQuote543 &_writer)
{
    RequestForQuote543 writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "RequestForQuote543", )";
...
void skip()
{
    auto &noRelatedSymGroup { noRelatedSym() };
    while (noRelatedSymGroup.hasNext())
    {
        noRelatedSymGroup.next().skip();
    }
}
