...
#ifndef _SBE_DECIMAL64NULL_CXX_H_
#define _SBE_DECIMAL64NULL_CXX_H_
...
public:
    ...
    Decimal64NULL(
        ...
        const std::uint64_t bufferLength,
        const std::uint64_t actingVersion) :
        Decimal64NULL(buffer, 0, bufferLength, actingVersion)
    {
    }
    ...
    Decimal64NULL(
        char *buffer,
        const std::uint64_t bufferLength) :
        Decimal64NULL(buffer, 0, bufferLength, sbeSchemaVersion())
    {
    }
    ...
    Decimal64NULL &wrap(
        ...
        const std::uint64_t bufferLength)
    {
        return *this = Decimal64NULL(buffer, offset, bufferLength, actingVersion);
    }
    ...
    Decimal64NULL &mantissa(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    Decimal64NULL &exponent(const std::int8_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, Decimal64NULL &writer)
{
    builder << '{';
    builder << R"("mantissa": )";
