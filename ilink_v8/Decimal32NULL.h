...
#ifndef _SBE_DECIMAL32NULL_CXX_H_
#define _SBE_DECIMAL32NULL_CXX_H_
...
public:
    ...
    Decimal32NULL(
        ...
        const std::uint64_t bufferLength,
        const std::uint64_t actingVersion) :
        Decimal32NULL(buffer, 0, bufferLength, actingVersion)
    {
    }
    ...
    Decimal32NULL(
        char *buffer,
        const std::uint64_t bufferLength) :
        Decimal32NULL(buffer, 0, bufferLength, sbeSchemaVersion())
    {
    }
    ...
    Decimal32NULL &wrap(
        ...
        const std::uint64_t bufferLength)
    {
        return *this = Decimal32NULL(buffer, offset, bufferLength, actingVersion);
    }
    ...
    SBE_NODISCARD std::int32_t mantissa() const SBE_NOEXCEPT
    {
        std::int32_t val;
    ...
    Decimal32NULL &mantissa(const std::int32_t value) SBE_NOEXCEPT
    {
    ...
    {
        std::int8_t val;
        std::memcpy(&val, m_buffer + m_offset + 4, sizeof(std::int8_t));
        return (val);
    }

    Decimal32NULL &exponent(const std::int8_t value) SBE_NOEXCEPT
    {
        std::int8_t val = (value);
        std::memcpy(m_buffer + m_offset + 4, &val, sizeof(std::int8_t));
        return *this;
    }

template<typename CharT, typename Traits>
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, Decimal32NULL &writer)
{
    builder << '{';
    builder << R"("mantissa": )";
