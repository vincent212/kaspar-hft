...
#ifndef _SBE_RETRANSMISSION509_CXX_H_
#define _SBE_RETRANSMISSION509_CXX_H_
...
public:
    static const std::uint16_t SBE_BLOCK_LENGTH = static_cast<std::uint16_t>(31);
    static const std::uint16_t SBE_TEMPLATE_ID = static_cast<std::uint16_t>(509);
    ...
    Retransmission509(char *buffer, const std::uint64_t bufferLength) :
        Retransmission509(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    Retransmission509(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        Retransmission509(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    {
        return static_cast<std::uint16_t>(509);
    }
    ...
    Retransmission509 &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        return *this = Retransmission509(buffer, offset, bufferLength, sbeBlockLength(), sbeSchemaVersion());
    }

    Retransmission509 &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    Retransmission509 &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
        const std::uint64_t bufferLength)
    {
        return *this = Retransmission509(buffer, offset, bufferLength, actingBlockLength, actingVersion);
    }

    Retransmission509 &sbeRewind()
    {
    ...
    {
        Retransmission509 skipper(m_buffer, m_offset, m_bufferLength, sbeBlockLength(), m_actingVersion);
        skipper.skip();
    ...
    Retransmission509 &uUID(const std::uint64_t value) SBE_NOEXCEPT
    {
    ...
    Retransmission509 &lastUUID(const std::uint64_t value) SBE_NOEXCEPT
    {
        std::uint64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    Retransmission509 &requestTimestamp(const std::uint64_t value) SBE_NOEXCEPT
    {
        std::uint64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    Retransmission509 &fromSeqNo(const std::uint32_t value) SBE_NOEXCEPT
    {
    ...
    Retransmission509 &msgCount(const std::uint16_t value) SBE_NOEXCEPT
    {
    ...
    Retransmission509 &splitMsg(const SplitMsg::Value value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const Retransmission509 &_writer)
{
    Retransmission509 writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "Retransmission509", )";
