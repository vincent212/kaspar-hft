...
#ifndef _SBE_TERMINATE507_CXX_H_
#define _SBE_TERMINATE507_CXX_H_
...
public:
    static const std::uint16_t SBE_BLOCK_LENGTH = static_cast<std::uint16_t>(67);
    static const std::uint16_t SBE_TEMPLATE_ID = static_cast<std::uint16_t>(507);
    ...
    Terminate507(char *buffer, const std::uint64_t bufferLength) :
        Terminate507(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    Terminate507(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        Terminate507(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    {
        return static_cast<std::uint16_t>(507);
    }
    ...
    Terminate507 &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        return *this = Terminate507(buffer, offset, bufferLength, sbeBlockLength(), sbeSchemaVersion());
    }

    Terminate507 &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    Terminate507 &wrapForDecode(
        ...
        const std::uint64_t bufferLength)
    {
        return *this = Terminate507(buffer, offset, bufferLength, actingBlockLength, actingVersion);
    }

    Terminate507 &sbeRewind()
    {
    ...
    {
        Terminate507 skipper(m_buffer, m_offset, m_bufferLength, sbeBlockLength(), m_actingVersion);
        skipper.skip();
    ...
    Terminate507 &reason(const std::uint64_t index, const char value)
    {
        if (index >= 48)
        {
    ...
    Terminate507 &putReason(const char *const src) SBE_NOEXCEPT
    {
    ...
    #if __cplusplus >= 201703L
    Terminate507 &putReason(const std::string_view str)
    {
    ...
    #else
    Terminate507 &putReason(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 48)
        {
    ...
    Terminate507 &uUID(const std::uint64_t value) SBE_NOEXCEPT
    {
    ...
    Terminate507 &requestTimestamp(const std::uint64_t value) SBE_NOEXCEPT
    {
        std::uint64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    Terminate507 &errorCodes(const std::uint16_t value) SBE_NOEXCEPT
    {
    ...
    Terminate507 &splitMsg(const SplitMsg::Value value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const Terminate507 &_writer)
{
    Terminate507 writer(
        _writer.m_buffer,
        _writer.m_offset,
