...
#ifndef _SBE_EXECUTIONACK539_CXX_H_
#define _SBE_EXECUTIONACK539_CXX_H_
...
public:
    static const std::uint16_t SBE_BLOCK_LENGTH = static_cast<std::uint16_t>(101);
    static const std::uint16_t SBE_TEMPLATE_ID = static_cast<std::uint16_t>(539);
    ...
    ExecutionAck539(char *buffer, const std::uint64_t bufferLength) :
        ExecutionAck539(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    ExecutionAck539(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        ExecutionAck539(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    {
        return static_cast<std::uint16_t>(539);
    }
    ...
    ExecutionAck539 &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        return *this = ExecutionAck539(buffer, offset, bufferLength, sbeBlockLength(), sbeSchemaVersion());
    }

    ExecutionAck539 &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    ExecutionAck539 &wrapForDecode(
        ...
        const std::uint64_t bufferLength)
    {
        return *this = ExecutionAck539(buffer, offset, bufferLength, actingBlockLength, actingVersion);
    }

    ExecutionAck539 &sbeRewind()
    {
    ...
    {
        ExecutionAck539 skipper(m_buffer, m_offset, m_bufferLength, sbeBlockLength(), m_actingVersion);
        skipper.skip();
    ...
    ExecutionAck539 &partyDetailsListReqID(const std::uint64_t value) SBE_NOEXCEPT
    {
    ...
    ExecutionAck539 &orderID(const std::uint64_t value) SBE_NOEXCEPT
    {
        std::uint64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    SBE_NODISCARD static const char *ExecAckStatusMetaAttribute(const MetaAttribute metaAttribute) SBE_NOEXCEPT
    {
        switch (metaAttribute)
        {
            case MetaAttribute::SEMANTIC_TYPE: return "int";
    ...
    static SBE_CONSTEXPR std::uint16_t execAckStatusId() SBE_NOEXCEPT
    {
        return 1036;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::uint64_t execAckStatusSinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }

    SBE_NODISCARD bool execAckStatusInActingVersion() SBE_NOEXCEPT
    {
        return true;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::size_t execAckStatusEncodingOffset() SBE_NOEXCEPT
    {
        return 16;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::size_t execAckStatusEncodingLength() SBE_NOEXCEPT
    {
        return 1;
    }

    SBE_NODISCARD std::uint8_t execAckStatusRaw() const SBE_NOEXCEPT
    {
        std::uint8_t val;
    ...
    SBE_NODISCARD ExecAckStatus::Value execAckStatus() const
    {
        std::uint8_t val;
        std::memcpy(&val, m_buffer + m_offset + 16, sizeof(std::uint8_t));
        return ExecAckStatus::get((val));
    }

    ExecutionAck539 &execAckStatus(const ExecAckStatus::Value value) SBE_NOEXCEPT
    {
    ...
    ExecutionAck539 &seqNum(const std::uint32_t value) SBE_NOEXCEPT
    {
    ...
    ExecutionAck539 &clOrdID(const std::uint64_t index, const char value)
    {
        if (index >= 20)
        {
    ...
    ExecutionAck539 &putClOrdID(const char *const src) SBE_NOEXCEPT
    {
    ...
    #if __cplusplus >= 201703L
    ExecutionAck539 &putClOrdID(const std::string_view str)
    {
    ...
    #else
    ExecutionAck539 &putClOrdID(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 20)
        {
    ...
    ExecutionAck539 &secExecID(const std::uint64_t value) SBE_NOEXCEPT
    {
        std::uint64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
...
public:
    ...
    {
        m_lastPx.wrap(m_buffer, m_offset + 49, m_actingVersion, m_bufferLength);
        return m_lastPx;
    }
    ...
    {
        std::int32_t val;
        std::memcpy(&val, m_buffer + m_offset + 57, sizeof(std::int32_t));
        return SBE_LITTLE_ENDIAN_ENCODE_32(val);
    }

    ExecutionAck539 &securityID(const std::int32_t value) SBE_NOEXCEPT
    {
        std::int32_t val = SBE_LITTLE_ENDIAN_ENCODE_32(value);
        std::memcpy(m_buffer + m_offset + 57, &val, sizeof(std::int32_t));
        return *this;
    }
    ...
    ExecutionAck539 &lastQty(const std::uint32_t value) SBE_NOEXCEPT
    {
        std::uint32_t val = SBE_LITTLE_ENDIAN_ENCODE_32(value);
    ...
    SBE_NODISCARD static const char *DKReasonMetaAttribute(const MetaAttribute metaAttribute) SBE_NOEXCEPT
    {
        switch (metaAttribute)
        {
    ...
    static SBE_CONSTEXPR std::uint16_t dKReasonId() SBE_NOEXCEPT
    {
        return 127;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::uint64_t dKReasonSinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }

    SBE_NODISCARD bool dKReasonInActingVersion() SBE_NOEXCEPT
    {
        return true;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::size_t dKReasonEncodingOffset() SBE_NOEXCEPT
    {
        return 65;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::size_t dKReasonEncodingLength() SBE_NOEXCEPT
    {
        return 1;
    }

    SBE_NODISCARD char dKReasonRaw() const SBE_NOEXCEPT
    {
        char val;
        std::memcpy(&val, m_buffer + m_offset + 65, sizeof(char));
        return (val);
    }

    SBE_NODISCARD DKReason::Value dKReason() const
    {
        char val;
        std::memcpy(&val, m_buffer + m_offset + 65, sizeof(char));
        return DKReason::get((val));
    }

    ExecutionAck539 &dKReason(const DKReason::Value value) SBE_NOEXCEPT
    {
        char val = (value);
        std::memcpy(m_buffer + m_offset + 65, &val, sizeof(char));
        return *this;
    }
    ...
    ExecutionAck539 &side(const SideReq::Value value) SBE_NOEXCEPT
    {
        std::uint8_t val = (value);
    ...
    SBE_NODISCARD char senderID(const std::uint64_t index) const
    {
        ...
        char val;
        std::memcpy(&val, m_buffer + m_offset + 67 + (index * 1), sizeof(char));
        return (val);
    }

    ExecutionAck539 &senderID(const std::uint64_t index, const char value)
    {
        if (index >= 20)
        {
        ...
        char val = (value);
        std::memcpy(m_buffer + m_offset + 67 + (index * 1), &val, sizeof(char));
        return *this;
    }
    ...
    std::uint64_t getSenderID(char *const dst, const std::uint64_t length) const
    {
        ...
        std::memcpy(dst, m_buffer + m_offset + 67, sizeof(char) * static_cast<std::size_t>(length));
        return length;
    }

    ExecutionAck539 &putSenderID(const char *const src) SBE_NOEXCEPT
    {
        std::memcpy(m_buffer + m_offset + 67, src, sizeof(char) * 20);
        return *this;
    }

    SBE_NODISCARD std::string getSenderIDAsString() const
    {
        const char *buffer = m_buffer + m_offset + 67;
        std::size_t length = 0;
    ...
    #if __cplusplus >= 201703L
    ExecutionAck539 &putSenderID(const std::string_view str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 20)
        {
        ...
        std::memcpy(m_buffer + m_offset + 67, str.data(), srcLength);
        for (std::size_t start = srcLength; start < 20; ++start)
        {
            m_buffer[m_offset + 67 + start] = 0;
        }

        return *this;
    }
    #else
    ExecutionAck539 &putSenderID(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 20)
        {
        ...
        std::memcpy(m_buffer + m_offset + 67, str.c_str(), srcLength);
        for (std::size_t start = srcLength; start < 20; ++start)
        {
            m_buffer[m_offset + 67 + start] = 0;
        }
    ...
    SBE_NODISCARD std::uint64_t sendingTimeEpoch() const SBE_NOEXCEPT
    {
        ...
        std::uint64_t val;
        std::memcpy(&val, m_buffer + m_offset + 87, sizeof(std::uint64_t));
        return SBE_LITTLE_ENDIAN_ENCODE_64(val);
    }

    ExecutionAck539 &sendingTimeEpoch(const std::uint64_t value) SBE_NOEXCEPT
    {
        std::uint64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
        std::memcpy(m_buffer + m_offset + 87, &val, sizeof(std::uint64_t));
        return *this;
    }
    ...
    ExecutionAck539 &location(const std::uint64_t index, const char value)
    {
        if (index >= 5)
        {
    ...
    ExecutionAck539 &putLocation(const char *const src) SBE_NOEXCEPT
    {
    ...
    #if __cplusplus >= 201703L
    ExecutionAck539 &putLocation(const std::string_view str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 5)
        {
    ...
    #else
    ExecutionAck539 &putLocation(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 5)
        {
    ...
    SBE_NODISCARD ManualOrdIndReq::Value manualOrderIndicator() const
    {
        if (m_actingVersion < 5)
        {
            return ManualOrdIndReq::NULL_VALUE;
        }
    ...
    ExecutionAck539 &manualOrderIndicator(const ManualOrdIndReq::Value value) SBE_NOEXCEPT
    {
        std::uint8_t val = (value);
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const ExecutionAck539 &_writer)
{
    ExecutionAck539 writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "ExecutionAck539", )";
    ...
    builder << ", ";
    builder << R"("ExecAckStatus": )";
    builder << '"' << writer.execAckStatus() << '"';
