...
#ifndef _SBE_ESTABLISHMENTACK504_CXX_H_
#define _SBE_ESTABLISHMENTACK504_CXX_H_
...
public:
    static const std::uint16_t SBE_BLOCK_LENGTH = static_cast<std::uint16_t>(38);
    static const std::uint16_t SBE_TEMPLATE_ID = static_cast<std::uint16_t>(504);
    ...
    {
        return static_cast<std::uint16_t>(504);
    }
    ...
    {
        EstablishmentAck504 skipper(m_buffer, m_offset, m_bufferLength, sbeBlockLength(), m_actingVersion);
        skipper.skip();
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const EstablishmentAck504 &_writer)
{
    EstablishmentAck504 writer(
        _writer.m_buffer,
        _writer.m_offset,
