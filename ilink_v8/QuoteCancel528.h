...
#ifndef _SBE_QUOTECANCEL528_CXX_H_
#define _SBE_QUOTECANCEL528_CXX_H_
...
public:
    static const std::uint16_t SBE_BLOCK_LENGTH = static_cast<std::uint16_t>(61);
    static const std::uint16_t SBE_TEMPLATE_ID = static_cast<std::uint16_t>(528);
    ...
    {
        return static_cast<std::uint16_t>(528);
    }
    ...
    {
        QuoteCancel528 skipper(m_buffer, m_offset, m_bufferLength, sbeBlockLength(), m_actingVersion);
        skipper.skip();
    ...
    SBE_NODISCARD QuoteCxlTyp::Value quoteCancelType() const
    {
        std::uint8_t val;
        std::memcpy(&val, m_buffer + m_offset + 50, sizeof(std::uint8_t));
    ...
    QuoteCancel528 &origOrderUser(const std::uint64_t index, const char value)
    {
        if (index >= 8)
        {
    ...
    QuoteCancel528 &putOrigOrderUser(const char *const src) SBE_NOEXCEPT
    {
        std::memcpy(m_buffer + m_offset + 52, src, sizeof(char) * 8);
        return *this;
    }
    ...
    #if __cplusplus >= 201703L
    QuoteCancel528 &putOrigOrderUser(const std::string_view str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 8)
        {
    ...
    #else
    QuoteCancel528 &putOrigOrderUser(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 8)
        {
    ...
    QuoteCancel528 &quoteEntryOpen(const BooleanNULL::Value value) SBE_NOEXCEPT
    {
        std::uint8_t val = (value);
...
template<typename CharT, typename Traits>
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const QuoteCancel528 &_writer)
{
    QuoteCancel528 writer(
        _writer.m_buffer,
        _writer.m_offset,
