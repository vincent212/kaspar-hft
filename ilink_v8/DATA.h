...
#ifndef _SBE_DATA_CXX_H_
#define _SBE_DATA_CXX_H_
...
public:
    ...
    DATA(
        ...
        const std::uint64_t bufferLength,
        const std::uint64_t actingVersion) :
        DATA(buffer, 0, bufferLength, actingVersion)
    {
    }
    ...
    DATA(
        char *buffer,
        const std::uint64_t bufferLength) :
        DATA(buffer, 0, bufferLength, sbeSchemaVersion())
    {
    }
    ...
    DATA &wrap(
        ...
        const std::uint64_t bufferLength)
    {
        return *this = DATA(buffer, offset, bufferLength, actingVersion);
    }
    ...
    DATA &length(const std::uint16_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, DATA &writer)
{
    builder << '{';
    builder << R"("length": )";
