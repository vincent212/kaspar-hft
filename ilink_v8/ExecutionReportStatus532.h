...
#ifndef _SBE_EXECUTIONREPORTSTATUS532_CXX_H_
#define _SBE_EXECUTIONREPORTSTATUS532_CXX_H_
...
public:
    static const std::uint16_t SBE_BLOCK_LENGTH = static_cast<std::uint16_t>(513);
    static const std::uint16_t SBE_TEMPLATE_ID = static_cast<std::uint16_t>(532);
    ...
    {
        return static_cast<std::uint16_t>(513);
    }
    ...
    {
        return static_cast<std::uint16_t>(532);
    }
    ...
    ExecutionReportStatus532 &sbeRewind()
    {
    ...
    {
        ExecutionReportStatus532 skipper(m_buffer, m_offset, m_bufferLength, sbeBlockLength(), m_actingVersion);
        skipper.skip();
...
public:
    ...
    std::uint64_t getLocation(char *const dst, const std::uint64_t length) const
    {
        ...
        std::memcpy(dst, m_buffer + m_offset + 436, sizeof(char) * static_cast<std::size_t>(length));
        return length;
    }
    ...
    SBE_NODISCARD OrderStatus::Value ordStatus() const
    {
        char val;
        std::memcpy(&val, m_buffer + m_offset + 467, sizeof(char));
    ...
    {
        static const std::uint8_t execTypeValues[] = { 73, 0 };
    ...
    SBE_NODISCARD BooleanNULL::Value lastRptRequested() const
    {
        std::uint8_t val;
        std::memcpy(&val, m_buffer + m_offset + 473, sizeof(std::uint8_t));
...
public:
    ...
    {
        m_discretionPrice.wrap(m_buffer, m_offset + 480, m_actingVersion, m_bufferLength);
...
public:
    ...
    {
        m_reservationPrice.wrap(m_buffer, m_offset + 488, m_actingVersion, m_bufferLength);
    ...
    SBE_NODISCARD std::uint8_t priorityIndicator() const SBE_NOEXCEPT
    {
        ...
        std::uint8_t val;
        std::memcpy(&val, m_buffer + m_offset + 496, sizeof(std::uint8_t));
        return (val);
    }

    ExecutionReportStatus532 &priorityIndicator(const std::uint8_t value) SBE_NOEXCEPT
    {
        std::uint8_t val = (value);
        std::memcpy(m_buffer + m_offset + 496, &val, sizeof(std::uint8_t));
        return *this;
    }
    ...
    SBE_NODISCARD char origOrderUser(const std::uint64_t index) const
    {
        ...
        char val;
        std::memcpy(&val, m_buffer + m_offset + 497 + (index * 1), sizeof(char));
        return (val);
    }

    ExecutionReportStatus532 &origOrderUser(const std::uint64_t index, const char value)
    {
        if (index >= 8)
        {
        ...
        char val = (value);
        std::memcpy(m_buffer + m_offset + 497 + (index * 1), &val, sizeof(char));
        return *this;
    }
    ...
    std::uint64_t getOrigOrderUser(char *const dst, const std::uint64_t length) const
    {
        ...
        std::memcpy(dst, m_buffer + m_offset + 497, sizeof(char) * static_cast<std::size_t>(length));
        return length;
    }

    ExecutionReportStatus532 &putOrigOrderUser(const char *const src) SBE_NOEXCEPT
    {
        std::memcpy(m_buffer + m_offset + 497, src, sizeof(char) * 8);
        return *this;
    }

    SBE_NODISCARD std::string getOrigOrderUserAsString() const
    {
        const char *buffer = m_buffer + m_offset + 497;
        std::size_t length = 0;
    ...
    #if __cplusplus >= 201703L
    ExecutionReportStatus532 &putOrigOrderUser(const std::string_view str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 8)
        {
        ...
        std::memcpy(m_buffer + m_offset + 497, str.data(), srcLength);
        for (std::size_t start = srcLength; start < 8; ++start)
        {
            m_buffer[m_offset + 497 + start] = 0;
        }

        return *this;
    }
    #else
    ExecutionReportStatus532 &putOrigOrderUser(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 8)
        {
        ...
        std::memcpy(m_buffer + m_offset + 497, str.c_str(), srcLength);
        for (std::size_t start = srcLength; start < 8; ++start)
        {
            m_buffer[m_offset + 497 + start] = 0;
        }
    ...
    SBE_NODISCARD char cancelText(const std::uint64_t index) const
    {
        ...
        char val;
        std::memcpy(&val, m_buffer + m_offset + 505 + (index * 1), sizeof(char));
        return (val);
    }

    ExecutionReportStatus532 &cancelText(const std::uint64_t index, const char value)
    {
        if (index >= 8)
        {
        ...
        char val = (value);
        std::memcpy(m_buffer + m_offset + 505 + (index * 1), &val, sizeof(char));
        return *this;
    }
    ...
    std::uint64_t getCancelText(char *const dst, const std::uint64_t length) const
    {
        ...
        std::memcpy(dst, m_buffer + m_offset + 505, sizeof(char) * static_cast<std::size_t>(length));
        return length;
    }

    ExecutionReportStatus532 &putCancelText(const char *const src) SBE_NOEXCEPT
    {
        std::memcpy(m_buffer + m_offset + 505, src, sizeof(char) * 8);
        return *this;
    }

    SBE_NODISCARD std::string getCancelTextAsString() const
    {
        const char *buffer = m_buffer + m_offset + 505;
        std::size_t length = 0;
    ...
    #if __cplusplus >= 201703L
    ExecutionReportStatus532 &putCancelText(const std::string_view str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 8)
        {
        ...
        std::memcpy(m_buffer + m_offset + 505, str.data(), srcLength);
        for (std::size_t start = srcLength; start < 8; ++start)
        {
            m_buffer[m_offset + 505 + start] = 0;
        }

        return *this;
    }
    #else
    ExecutionReportStatus532 &putCancelText(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 8)
        {
        ...
        std::memcpy(m_buffer + m_offset + 505, str.c_str(), srcLength);
        for (std::size_t start = srcLength; start < 8; ++start)
        {
            m_buffer[m_offset + 505 + start] = 0;
        }
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const ExecutionReportStatus532 &_writer)
{
    ExecutionReportStatus532 writer(
        _writer.m_buffer,
        _writer.m_offset,
