...
#ifndef _SBE_NOTAPPLIED513_CXX_H_
#define _SBE_NOTAPPLIED513_CXX_H_
...
public:
    ...
    static const std::uint16_t SBE_TEMPLATE_ID = static_cast<std::uint16_t>(513);
    ...
    NotApplied513(char *buffer, const std::uint64_t bufferLength) :
        NotApplied513(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    NotApplied513(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        NotApplied513(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    NotApplied513 &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        return *this = NotApplied513(buffer, offset, bufferLength, sbeBlockLength(), sbeSchemaVersion());
    }

    NotApplied513 &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    NotApplied513 &wrapForDecode(
        ...
        const std::uint64_t bufferLength)
    {
        return *this = NotApplied513(buffer, offset, bufferLength, actingBlockLength, actingVersion);
    }

    NotApplied513 &sbeRewind()
    {
    ...
    {
        NotApplied513 skipper(m_buffer, m_offset, m_bufferLength, sbeBlockLength(), m_actingVersion);
        skipper.skip();
    ...
    NotApplied513 &uUID(const std::uint64_t value) SBE_NOEXCEPT
    {
    ...
    NotApplied513 &fromSeqNo(const std::uint32_t value) SBE_NOEXCEPT
    {
    ...
    static SBE_CONSTEXPR std::uint32_t msgCountNullValue() SBE_NOEXCEPT
    {
        return SBE_NULLVALUE_UINT32;
    }

    static SBE_CONSTEXPR std::uint32_t msgCountMinValue() SBE_NOEXCEPT
    {
        return UINT32_C(0x0);
    }

    static SBE_CONSTEXPR std::uint32_t msgCountMaxValue() SBE_NOEXCEPT
    {
        return UINT32_C(0xfffffffe);
    }
    ...
    SBE_NODISCARD std::uint32_t msgCount() const SBE_NOEXCEPT
    {
        std::uint32_t val;
    ...
    NotApplied513 &msgCount(const std::uint32_t value) SBE_NOEXCEPT
    {
        std::uint32_t val = SBE_LITTLE_ENDIAN_ENCODE_32(value);
    ...
    NotApplied513 &splitMsg(const SplitMsg::Value value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const NotApplied513 &_writer)
{
    NotApplied513 writer(
        _writer.m_buffer,
        _writer.m_offset,
