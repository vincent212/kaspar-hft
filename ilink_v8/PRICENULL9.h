...
#ifndef _SBE_PRICENULL9_CXX_H_
#define _SBE_PRICENULL9_CXX_H_
...
public:
    ...
    PRICENULL9(
        ...
        const std::uint64_t bufferLength,
        const std::uint64_t actingVersion) :
        PRICENULL9(buffer, 0, bufferLength, actingVersion)
    {
    }
    ...
    PRICENULL9(
        char *buffer,
        const std::uint64_t bufferLength) :
        PRICENULL9(buffer, 0, bufferLength, sbeSchemaVersion())
    {
    }
    ...
    PRICENULL9 &wrap(
        ...
        const std::uint64_t bufferLength)
    {
        return *this = PRICENULL9(buffer, offset, bufferLength, actingVersion);
    }
    ...
    PRICENULL9 &mantissa(const std::int64_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, PRICENULL9 &writer)
{
    builder << '{';
    builder << R"("mantissa": )";
