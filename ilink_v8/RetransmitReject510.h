...
#ifndef _SBE_RETRANSMITREJECT510_CXX_H_
#define _SBE_RETRANSMITREJECT510_CXX_H_
...
public:
    static const std::uint16_t SBE_BLOCK_LENGTH = static_cast<std::uint16_t>(75);
    static const std::uint16_t SBE_TEMPLATE_ID = static_cast<std::uint16_t>(510);
    ...
    RetransmitReject510(char *buffer, const std::uint64_t bufferLength) :
        RetransmitReject510(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    RetransmitReject510(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        RetransmitReject510(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::uint16_t sbeBlockLength() SBE_NOEXCEPT
    {
        return static_cast<std::uint16_t>(75);
    }
    ...
    {
        return static_cast<std::uint16_t>(510);
    }
    ...
    RetransmitReject510 &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        return *this = RetransmitReject510(buffer, offset, bufferLength, sbeBlockLength(), sbeSchemaVersion());
    }

    RetransmitReject510 &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        ...
        return *this = RetransmitReject510(
            buffer,
    ...
    RetransmitReject510 &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
        const std::uint64_t bufferLength)
    {
        return *this = RetransmitReject510(buffer, offset, bufferLength, actingBlockLength, actingVersion);
    }

    RetransmitReject510 &sbeRewind()
    {
    ...
    {
        RetransmitReject510 skipper(m_buffer, m_offset, m_bufferLength, sbeBlockLength(), m_actingVersion);
        skipper.skip();
    ...
    RetransmitReject510 &reason(const std::uint64_t index, const char value)
    {
        if (index >= 48)
        {
    ...
    RetransmitReject510 &putReason(const char *const src) SBE_NOEXCEPT
    {
    ...
    #if __cplusplus >= 201703L
    RetransmitReject510 &putReason(const std::string_view str)
    {
    ...
    #else
    RetransmitReject510 &putReason(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 48)
        {
    ...
    RetransmitReject510 &uUID(const std::uint64_t value) SBE_NOEXCEPT
    {
    ...
    RetransmitReject510 &lastUUID(const std::uint64_t value) SBE_NOEXCEPT
    {
        std::uint64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    RetransmitReject510 &requestTimestamp(const std::uint64_t value) SBE_NOEXCEPT
    {
        std::uint64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    RetransmitReject510 &errorCodes(const std::uint16_t value) SBE_NOEXCEPT
    {
    ...
    RetransmitReject510 &splitMsg(const SplitMsg::Value value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const RetransmitReject510 &_writer)
{
    RetransmitReject510 writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "RetransmitReject510", )";
