...
#ifndef _SBE_QUOTECXLTYP_CXX_H_
#define _SBE_QUOTECXLTYP_CXX_H_
...
public:
    ...
    static QuoteCxlTyp::Value get(const std::uint8_t value)
    {
        switch (value)
        {
        ...
        throw std::runtime_error("unknown value for enum QuoteCxlTyp [E103]");
    }

    static const char *c_str(const QuoteCxlTyp::Value value)
    {
        switch (value)
        {
    ...
    friend std::basic_ostream<CharT, Traits> & operator << (
        std::basic_ostream<CharT, Traits> &os, QuoteCxlTyp::Value m)
    {
        return os << QuoteCxlTyp::c_str(m);
    }
};
