...
#ifndef _SBE_ORDERCANCELREQUEST516_CXX_H_
#define _SBE_ORDERCANCELREQUEST516_CXX_H_
...
public:
    static const std::uint16_t SBE_BLOCK_LENGTH = static_cast<std::uint16_t>(96);
    static const std::uint16_t SBE_TEMPLATE_ID = static_cast<std::uint16_t>(516);
    ...
    {
        return static_cast<std::uint16_t>(96);
    }
    ...
    {
        return static_cast<std::uint16_t>(516);
    }
    ...
    OrderCancelRequest516 &sbeRewind()
    {
    ...
    {
        OrderCancelRequest516 skipper(m_buffer, m_offset, m_bufferLength, sbeBlockLength(), m_actingVersion);
        skipper.skip();
    ...
    OrderCancelRequest516 &origOrderUser(const std::uint64_t index, const char value)
    {
        if (index >= 8)
        {
    ...
    OrderCancelRequest516 &putOrigOrderUser(const char *const src) SBE_NOEXCEPT
    {
        std::memcpy(m_buffer + m_offset + 88, src, sizeof(char) * 8);
        return *this;
    }
    ...
    #if __cplusplus >= 201703L
    OrderCancelRequest516 &putOrigOrderUser(const std::string_view str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 8)
        {
    ...
    #else
    OrderCancelRequest516 &putOrigOrderUser(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 8)
        {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const OrderCancelRequest516 &_writer)
{
    OrderCancelRequest516 writer(
        _writer.m_buffer,
        _writer.m_offset,
