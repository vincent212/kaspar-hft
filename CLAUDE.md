...
The CME SBE codecs (`mktdata_v12/`, `ilink_v8/`) are **generated by the build, not committed** — they come from CME's SBE templates via `genschema/genschema.py` (see `genschema/README.md`). On a fresh checkout, generate them first (pinned to the tested MDP3 v12 / iLink v8):
