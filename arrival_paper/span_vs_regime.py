...
"""Standalone span-vs-regime diagnostic for paper §5.5.

For each 30-minute RTH window in each session:
    * group SBE messages by unique sendingTime T_g to get packets and spans
    * fit exp-Hawkes on the packet arrival stream {T_g_pkt} -> (lambda_bar_pkt, n_pkt)
    * record mean span, p95 span, max span
    * record message count

Output per-session parquet + a corpus-level 5x5 (lambda_bar_pkt, n_pkt) heatmap
of median span_mean per cell.

Hypothesis: span grows with intensity and n. If true, packet-level burstiness
translates into decoder-serving load, and the qsim's span-scaled service time
is essential.

CLI:
    python3 -m arrival_paper.span_vs_regime \\
        --tapes-dir /vast/home/vmayeski/out/arrival_paper/tapes/318/message \\
        --out-dir arrival_paper/figs/span_regime \\
        --jobs 40
"""
...
MIN_PACKETS_PER_WINDOW = 500      # need enough packets for a stable Hawkes fit
MIN_MESSAGES_PER_WINDOW = 1000
CHECKPOINT_EVERY = 25             # write partial parquet + heatmaps every N sessions
...
def process_session(msg_tape_csv: str) -> pd.DataFrame:
    """Read one msgtape, return a DataFrame with one row per 30-min window."""
    tt: list[int] = []
    st: list[int] = []
    ...
    # sort by transactTime (the natural session ordering)
    order = np.argsort(tt_arr, kind="stable")
    tt_arr = tt_arr[order]
    st_arr = st_arr[order]

    # Skip messages with sendingTime = 0 (rare corrupted rows).
    mask = st_arr > 0
    tt_arr = tt_arr[mask]
    st_arr = st_arr[mask]
    if len(tt_arr) == 0:
    ...
    rows = []
    session = Path(msg_tape_csv).stem   # e.g. 20250408.NQM5
    for wid in np.unique(win_idx):
        ...
        # Packet grouping via unique sendingTime.
        pkt_start, spans = np.unique(st_w, return_counts=True)
        n_packets = len(pkt_start)
        if n_packets < MIN_PACKETS_PER_WINDOW:
            continue

        # Hawkes fit on packet arrivals (sendingTime).
        pkt_start = np.sort(pkt_start)          # already sorted by unique, defensive
        try:
        ...
        rows.append({
            ...
            "span_p50": float(np.quantile(spans, 0.50)),
...
def write_checkpoint(parts, out: Path, grid: int, min_cells: int,
                     tag: str = "[span]", final: bool = False) -> None:
    """Write partial parquet + heatmaps from parts accumulated so far."""
    if not parts:
        return
    corpus = pd.concat(parts, ignore_index=True)
    out.mkdir(parents=True, exist_ok=True)
    corpus.to_parquet(out / "span_regime.parquet", compression="snappy", index=False)
    prefix = "FINAL" if final else "CKPT"
    print(f"{tag} {prefix} wrote span_regime.parquet ({len(corpus):,} rows)",
          file=sys.stderr)
    ...
    for stat, label in [("span_mean", "median cell span_mean (msgs/pkt)"),
                        ("span_p95",  "median cell span_p95  (msgs/pkt)"),
                        ("span_max",  "median cell span_max  (msgs/pkt)")]:
        ...
        title = f"{stat} across (lambda_bar_pkt, n_pkt) — median across cell windows"
    ...
    plot_heatmap(mass.astype(float), n_edges, lam_edges,
                 f"Cell mass — # windows per cell (total {mass.sum():,})",
                 out / "grid_mass.png", cbar_label="# windows")

    for stat in ("span_mean", "span_p95", "span_max"):
        rho_lam = df[["lambda_bar_pkt", stat]].corr(method="spearman").iloc[0, 1]
        rho_n   = df[["n_branch_pkt",   stat]].corr(method="spearman").iloc[0, 1]
        print(f"{tag} {prefix} spearman({stat}, lambda_bar_pkt)={rho_lam:+.3f}  "
              f"({stat}, n_branch_pkt)={rho_n:+.3f}",
              file=sys.stderr)
...
def plot_heatmap(cells: np.ndarray, x_edges: np.ndarray, y_edges: np.ndarray,
    ...
    ax.set_xticklabels([f"{e:.2g}" for e in x_edges[:-1]], fontsize=8, rotation=0)
    ax.set_yticklabels([f"{e:.2g}" for e in y_edges[:-1]], fontsize=8)
    ax.set_xlabel("Hawkes branching ratio n (packets)")
    ax.set_ylabel("Packet mean intensity lambda_bar (pkt/sec)")
    ax.set_title(title)
...
def main() -> int:
    ...
    print(f"[span] found {len(tapes)} session tapes; jobs={args.jobs}",
          file=sys.stderr)
    if not tapes:
        return 1
    ...
    with mp.Pool(args.jobs) as pool:
        ...
        for i, df in enumerate(pool.imap_unordered(process_session, tapes)):
            if len(df):
                parts.append(df)
            print(f"[span] {i+1}/{len(tapes)}  cumulative rows={sum(len(p) for p in parts):,}",
                  file=sys.stderr)
            if (i + 1) % CHECKPOINT_EVERY == 0:
                write_checkpoint(parts, out, args.grid, args.min_cells,
                                 tag="[span]", final=False)
    if not parts:
        print("[span] no usable windows", file=sys.stderr)
        return 1
    write_checkpoint(parts, out, args.grid, args.min_cells,
                     tag="[span]", final=True)
    return 0
