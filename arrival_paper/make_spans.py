...
Output: <out-dir>/{session}.npz keyed w{window_id}_S (int64 message counts).
...
def spans_for_session(msg_tape_csv: str, out_dir: str,
                      window_min: int = DEFAULT_WINDOW_MIN) -> tuple[str, dict]:
...
def _worker(a: tuple[str, str, int]) -> tuple[str, dict]:
...
def main() -> int:
    ...
    with mp.Pool(args.jobs) as pool:
        for i, (session, counts) in enumerate(pool.imap_unordered(
                _worker, [(t, str(out), args.window_min) for t in tapes],
                chunksize=1)):
