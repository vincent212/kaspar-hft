...
\author{Vincent Maciejewski\thanks{M2 Tech (16425640 Canada Inc.), Montreal. \texttt{mayeski@gmail.com}. \kasparhft{} is open source at \url{https://github.com/vincent212/kaspar-hft}.}}
...
\textbf{First: the thread-hop cost is a wash.} The received single-thread wisdom assumes that a $\sim 1.7\,\mu\mathrm{s}$ one-way async mailbox hop (\citet{mayeski_fastsend} Table 4; the fast\_send paper's $3.4\,\mu\mathrm{s}$ round-trip halved) is a material addition to end-to-end latency. On real Hawkes-clustered CME MDP3 packet arrivals, the observed cluster-wait tail is one to two orders of magnitude larger than that per-hop cost at every service floor from $4$ to $32\,\mu\mathrm{s}$, so the hop is not a first-order effect. The single-thread objection --- ``every hop is a fixed tax that no benefit can overcome'' --- is empirically wrong on this class of feed.

\textbf{Second: two threads beat one on the tail at the cost of one hop on the median.} Splitting a service task into two half-length actors on separate threads joined by one $1.7\,\mu\mathrm{s}$ mailbox hop lowers $p_{99}$ on $100\%$ of 3512 thirty-minute windows across 276 sessions at every service floor $T$ from $4$ to $32\,\mu\mathrm{s}$, cutting the corpus-median tail excess to $0.28$--$0.34$ of its single-thread value while adding exactly $h$ to the median; below the $2h$ floor, at $T = 2\,\mu\mathrm{s}$, it loses, as the design equation predicts. The $p_{99}$-minimising stage count rises with the task length: $2$ at $4\,\mu\mathrm{s}$, $4$ at $8$--$16\,\mu\mathrm{s}$, $8$ at $32\,\mu\mathrm{s}$. The same pattern generalises to an $N$-stage series tandem: the end-to-end wait is at most $1/N$ of the single-stage wait plus $(N-1)h$, at every quantile and on every sample path, capped by the number of natural cut points in the decode chain (four or five on CME MDP3).

The effect is entirely arrival-law-driven: under a rate-matched Poisson null on the same arrival count, both wins vanish and the multi-actor architecture strictly loses. Two closed-form results anchor the mechanism (a burst-limit throughput identity, and an exact pathwise reduction of the deterministic-service tandem to a single server that yields the $1/N$ bound); a literature search over foundational tandem theory, Hawkes-adjacent queueing, and empirical HFT/RPC work found no direct prior art on either the theoretical statement or the empirical calibration. Companion code contains the message-tape parser, the per-window packet-Hawkes fitter, and the tandem Lindley simulator.
\end{abstract}
...
\begin{itemize}
    \item \textbf{Theory.} Two closed-form results (Section~\ref{sec:theory}) predict the pipeline behaviour. Proposition~\ref{thm:burst} shows that under any arrival process an $N$-stage tandem enjoys an $N$-factor throughput speedup on isolated bursts. Theorem~\ref{thm:reduction} shows that a deterministic-service tandem reduces exactly to a single server with the largest stage's service, and that the end-to-end wait is bounded pathwise by $1/N$ of the single-stage wait plus $(N-1)h$, at every quantile and under every arrival sequence. Corollary~\ref{cor:poisson} shows that under a rate-matched Poisson null at the feed's utilisation the single-stage wait is already zero, so there is nothing to compress and splitting costs exactly $(N-1)h$. The Hawkes cluster representation then explains why the single-stage wait on a real feed is two orders of magnitude above $T$: the Borel cluster size has scale $(1-n)^{-2}$.
    \item \textbf{Simulation.} We build an $N$-stage tandem Lindley recursion on real CME MDP3 packet-arrival streams at a hop cost $h = 1.7\,\mu\mathrm{s}$ (the one-way async mailbox cost derived by halving the round-trip figure in Table 4 of \citet{mayeski_fastsend}, and consistent with \citet{mayeski_actorhft}) and a service floor swept across $T \in \{2, 4, 8, 16, 32\}\,\mu\mathrm{s}$ to cover the regime from tasks below the hop cost (where no split helps) through tasks well above it (where the tail-win compounds). We sweep $N \in \{1, 2, 4, 8\}$ per scenario on each of the 3512 thirty-minute RTH windows of the corpus. Each window is also simulated under a uniform-shuffle Poisson null of the same arrival count as an empirical counterfactual (Section~\ref{sec:setup-sweep}).
    \item \textbf{Design equation for practitioners.} From the empirical result we extract a stage-count rule in terms of the operator's tail weight and the largest stage of the partition, and work it through on the \kasparhft{} actor chain that carries the \shadowpov{} execution algorithm \citep{maciejewski_shadowpov} (Sections~\ref{sec:kaspar} and~\ref{sec:implementation}).
\end{itemize}

\subsection{Contribution and prior-art positioning}
\label{sec:intro-priors}

Two rounds of literature search covering foundational tandem theory (Jackson, Kelly, Burke, Kleinrock, Whitt), tandem theory under bursty arrivals (Foss--Korshunov, Baccelli--Foss--Lelarge, Heindl, Kim--Chydzinski, D\k{e}bicki--Mandjes), Hawkes single-queue and adjacent (Daw--Pender, Chen--Blanchet, Koops--Boxma--Mandjes, Sheldon et~al., Gao--Zhu, Bo Li, and 2023--2026 follow-ons), and empirical/applied HFT and RPC-pipeline work (ABIDES, ABIDES-MARL, JAX-LOB, Aquilina--Budish--O'Neill, Dean--Barroso, Parsimon, and the datacenter tail-latency crowd) confirmed that there is no direct prior art on either (i) the theoretical claim that the tandem-vs-single-queue architecture recommendation flips sign as a function of arrival law (Poisson vs Hawkes) or (ii) an empirical CME-calibrated tandem sweep with a Poisson null and measured HFT-framework hop cost. The full reference list appears in Section~\ref{sec:refs}.

\subsection{Note on the use of AI in this paper}
\label{sec:intro-ai-use}

This paper was produced with Anthropic's Claude (via Claude Code) used under the author's direction for code generation --- the message-tape parser, the packet-Hawkes fitter, and the tandem-Lindley simulator run on the 3512-window corpus --- and for exploratory analysis, mathematical drafting, prior-art literature search, prose editing, and figure preparation. The analytical results of Part~I, including the proof of Theorem~\ref{thm:reduction}, were drafted with the model and then independently checked by the author. One episode is worth recording: an earlier model-drafted heavy-tail conjecture (an $N^{-1/2}$ tail-ratio built from the Borel--Tanner cluster tail and an $M^{[K]}/D/1$ single-big-batch principle) was found on review to be unsound --- the Borel law is light-tailed at every $n < 1$ (Appendix~\ref{app:borel}) --- and was replaced by the present, stronger reduction theorem. The author independently reviewed all mathematical arguments, experimental methodology, numerical results, and citations, and takes sole responsibility for the contents of the paper.
...
A baseline proposition, a theorem, and a corollary anchor the paper's simulation. Proposition~\ref{thm:burst} is the isolated-burst identity: an $N$-stage deterministic-service tandem clears a large burst $N$ times faster than one server. Theorem~\ref{thm:reduction} is the general statement: the tandem's end-to-end wait equals the wait at a single server with the largest stage's service plus $(N-1)h$, exactly and on every sample path, and that single-server wait is at most $1/N$ of the wait at service $T$. Both are elementary and arrival-law independent; the second rests on a reduction theorem of \citet{friedman1965} and on the monotonicity of the Lindley recursion. Corollary~\ref{cor:poisson} is the Poisson null: at the utilisation of a CME packet feed the single-stage wait is zero at every reported quantile, so splitting costs exactly $(N-1)h$. The arrival law enters only through the size of the single-stage wait that the $1/N$ is applied to, and the Hawkes--Oakes cluster representation explains why that wait is large on a real feed. The two rounds of literature search summarised in Section~\ref{sec:intro-priors} found no published statement of the architecture recommendation flipping with the arrival law; the results below are the paper's formal content, and Part~II measures the slack in the bound on real arrivals.
...
\begin{remark}
Proposition~\ref{thm:burst} is the classical pipelined-tandem throughput result \citep{kleinrock1976,kelly1979}; we state it without proof and use it only to fix the baseline against which the arrival-dependent tail reduction of Theorem~\ref{thm:reduction} is measured. What is paper-specific is the numerical calibration of $T$ and $h$ on a production HFT actor framework and the observation that under real CME MDP3 arrivals the ratio $\tau_k^{(1)}/\tau_k^{(N)}$ is what governs the observed tail-latency compression.
\end{remark}
...
Proposition~\ref{thm:burst} treats one isolated burst. The result that carries the paper's design claim is that the same $N$-factor holds pathwise for every message under every arrival sequence, at the level of the end-to-end wait. Fix an arrival sequence $a_1 \le a_2 \le \cdots$ (any point process, clustered or otherwise, with batches allowed as coincident times) and write $\delta_j = a_{j+1} - a_j \ge 0$. For a single FIFO server with deterministic service $s$ fed by this sequence, the waiting time of message $j$ obeys the Lindley recursion \citep{lindley1952}
\begin{equation}
...
\label{eq:bound}
\end{equation}
Since Equation~\eqref{eq:bound} holds pathwise, it holds for every quantile of the stationary distribution whenever one exists: $W_q^{(N)} \le W_q^{(1)}/N + (N-1)h$ for all $q$. The same inequality holds for the empirical (sample) quantiles computed over any finite trace, directly from the pathwise bound and without invoking stationarity; the sample-quantile form is what Part~II measures, and it should be read as a statement about the observed windows rather than about a stationary-distribution quantile or a finite-sample maximum.
\end{theorem}
...
\end{equation}
At the utilisations of a CME packet feed (corpus-median $452$ packets per second, so $\rho = 0.0036$ at $T = 8\,\mu\mathrm{s}$) this covers $p_{99}$ at every $T \le 16\,\mu\mathrm{s}$ in the paper's sweep: end-to-end latency is $T + (N-1)h$ at $p_{50}$ and at $p_{99}$ alike, and splitting costs $(N-1)h$ with nothing to compress. Where $\rho/N$ does exceed $1 - q$, Equation~\eqref{eq:bound} still holds and the tandem compresses the small Poisson wait too; the sweep contains exactly one such $p_{99}$ cell ($T = 32\,\mu\mathrm{s}$, $N = 1$, $\rho = 0.0145$) and several $p_{99.9}$ cells, and Section~\ref{sec:results-tail} shows the wait appearing in precisely those.
\end{corollary}

\subsection{What the theory says, in plain terms}
\label{sec:theory-summary}

This subsection restates Theorems~\ref{thm:burst} and~\ref{thm:reduction} and Corollary~\ref{cor:poisson} without the formalism, for readers who build pipelines rather than analyse queues. The formal statements above remain the reference.

\paragraph{Start from where a message's time goes.} A message's end-to-end latency is the sum of three things: the work done on it ($T$ in total, however many stages that work is spread over), the hop tax ($h$ for every mailbox boundary it crosses, so $(N-1)h$ in an $N$-stage pipeline), and the time it spends sitting in queues waiting for a busy actor. The paper calls that third piece the wait, $W$. The work is fixed by the code and the hop tax is fixed by the framework; the wait is the only piece the architecture can change, and on a bursty feed it is the piece that dominates the tail.

\paragraph{Proposition~\ref{thm:burst}: one burst.} Suppose $k$ messages land at once. On a single thread the last of them waits for the $k-1$ ahead of it, each costing $T$: it waits $(k-1)T$. On an $N$-stage pipeline the first stage takes only $T/N$ per message, so the last message waits $(k-1)T/N$ at stage~1 and then flows through the remaining stages without waiting again, because each of them is fed at exactly the rate it can serve. The pipeline clears a burst $N$ times faster for the same total work, because $N$ cores are working on $N$ different messages at the same instant. That is what pipelining means, and it is the whole mechanism.

\paragraph{Theorem~\ref{thm:reduction}: every message, whatever the traffic.} Two facts, each of which a programmer can check by hand.
\begin{enumerate}
    \item In a pipeline where each stage does a fixed amount of work per message, only the slowest stage ever builds a queue. Every other stage receives messages no faster than it can process them, because the slowest stage upstream (or, if the slowest stage is downstream, the stages before it) meters them out. So the whole pipeline behaves exactly like one thread whose per-message work is the slowest stage's work, $s_{\max}$, plus the hop tax. With equal stages, $s_{\max} = T/N$.
    \item Take any recording of arrival times, real or synthetic, and replay it through two single-threaded servers: one doing $T$ of work per message and one doing $T/N$. The second server's queue wait for message number $j$ is never more than $1/N$ of the first server's wait for the same message. The reason is that in the faster server every backlog is $N$ times smaller and every idle gap between arrivals clears at least as much of it.
\end{enumerate}
Put together: for every message in every replay, the pipeline's wait is at most the single-thread wait divided by $N$, plus $(N-1)h$. Because that holds message by message, it holds for whichever summary statistic the operator cares about: median, $p_{99}$, $p_{99.9}$, or the worst case. And the bound is usually beaten. It is met exactly on an isolated burst that arrives all at once, and beaten whenever arrivals are spread out, because the pipeline's first stage is idle more often than the single thread and uses each idle gap to drain more of its backlog. The paper writes the measured compression as $N^{-\gamma}$; the theorem guarantees $\gamma \ge 1$, and Section~\ref{sec:results} measures $\gamma$ between $1.0$ (at $T = 2\,\mu\mathrm{s}$, where the bound is tight) and $1.7$ (at $T = 32\,\mu\mathrm{s}$).

\paragraph{Where the traffic model comes in.} Nowhere in the two theorems. The arrival pattern enters only through the size of the single-thread wait that the factor $1/N$ is applied to. Under Poisson traffic at the load of a CME packet feed, the single thread is idle when almost every message arrives, so its wait is zero at every quantile the paper reports; there is nothing to divide by $N$, and the pipeline is simply $(N-1)h$ slower (Corollary~\ref{cor:poisson}). Under the real feed, packets arrive in self-exciting clusters, the single thread's $p_{99}$ is $2.7$ to $8$ times $T$ in the paper's data for tasks of $4$ to $32\,\mu\mathrm{s}$, and the pipeline divides the excess by $N$ or better. The Hawkes cluster representation says why the clusters are large: a cluster's size follows the Borel distribution, whose typical large cluster has of order $(1-n)^{-2}$ packets, about $25$ at the corpus's $n \approx 0.8$.

\paragraph{The rules a designer can take away.}
\begin{itemize}
    \item The tail improvement is set by the slowest stage, $s_{\max}/T$, and by nothing else about the decomposition. Balanced stages are the best case for a given number of hops.
    \item A cut that does not shrink the slowest stage costs one $h$ at every quantile and improves nothing. Sub-tasks can be merged onto one thread until their combined work reaches $s_{\max}$ at no tail cost.
    \item The order of the stages does not change any message's latency. It changes only where the queue physically sits; putting the slowest stage first keeps all queueing in one mailbox.
    \item Every hop costs exactly $h$ on every message, median and tail alike; there is no regime where the hop tax is hidden.
    \item Whether splitting is worth it comes down to one comparison: the single-thread wait at the quantile you care about, times $(1 - s_{\max}/T)$, against $h$ per hop added. On a Poisson-like feed the left side is zero; on a clustered feed it is tens of microseconds.
\end{itemize}

The guarantees above are exact inside a specific model, and a designer should know what that model leaves out. The rest of this section lists its assumptions and the questions it leaves to measurement.

\paragraph{Fixed work per message.} Each stage is assumed to spend the same amount of time on every message. That is what makes only the slowest stage queue: a stage that sometimes takes longer than its upstream neighbour would occasionally fall behind and build its own queue, and then the pipeline's wait would exceed the single-server wait at $s_{\max}$. The paper's simulator uses constant service, so the theorems describe the simulator exactly; real actors have data-dependent work (a book update touching a deep level costs more than one touching the top), and for them the theorems are a model to be checked by measurement, which is the paper's operational recommendation in any case (Section~\ref{sec:fungibility}).

\paragraph{No drops, no bounds, in-order.} Mailboxes are unbounded, messages are never discarded or coalesced, and each stage processes in arrival order. A framework that drops or merges messages under load changes the arithmetic (in the pipeline's favour, at the cost of lost messages); a priority queue changes which message pays the wait but not the total.

\paragraph{Fixed hop delay.} The hop is modelled as a constant $h$. A jittery hop lets a downstream stage queue by at most the jitter amplitude per message, so the reduction holds up to that amplitude. The paper's calibration uses the low-load, wake-from-sleep value of $h$, which Section~\ref{sec:results-h} argues is an upper bound inside a burst.

\paragraph{Proposition~\ref{thm:burst} needs a dense burst.} The one-burst identity assumes the $k$ messages arrive faster than the first stage can drain them, i.e.\ faster than one per $T/N$. For a burst that trickles in, the ratio in Equation~\eqref{eq:burst-limit} is $kN / (k + N - 1 + N(N-1)h/T)$, which climbs toward $N$ as the burst grows. Theorem~\ref{thm:reduction} does not need this assumption; it covers every arrival pattern.

\paragraph{What the theory does not predict.} It does not say how large the single-thread wait is on a given feed. That number depends on the feed's clustering and utilisation and, as the remark after the Borel fact explains, has no clean closed form at a fixed branching ratio: the Borel cluster size is light-tailed for every $n < 1$, so the heavy-tail formulas that would give a power-law exponent do not apply. The paper measures the single-thread wait on real CME arrivals instead (Part~II). Nor does the theory predict how far below the $1/N$ bound the pipeline lands; that slack is also measured. What the theory does guarantee is that the pipeline can never do worse than $1/N$ of the single-thread wait plus the hop tax, on any feed whatsoever, which is the statement a designer needs in order to trust the trade before measuring it.
...
Worked example. Three packets arriving at $t = 0, 1, 2\,\mu\mathrm{s}$; single-stage total service $T = 8\,\mu\mathrm{s}$ (a service floor in the sweep of Table~\ref{tab:main-corpus}); two-stage tandem with per-stage service $T/2 = 4\,\mu\mathrm{s}$ and hop $h = 1.7\,\mu\mathrm{s}$:
...
The design asks whether an $N$-stage tandem reduces $p_{99}$ latency under real Hawkes packet arrivals and whether the same architecture achieves the same compression under a rate-matched Poisson null. For each 30-minute window we run the tandem Lindley of Section~\ref{sec:setup-simulator} at $N \in \{1, 2, 4, 8\}$ under two arrival regimes: the real packet-arrival timestamps from the tape (Hawkes arm) and a uniform-shuffle of the same $n_{\mathrm{pkt}}^{\#}$ arrivals over the same support (Poisson-null arm). The support is the observed packet span $[\min \text{arr}_H, \max \text{arr}_H]$ rather than the nominal window $[T_0, T_1]$, so the null is rate-matched to the arrivals it replaces rather than to wall-clock; the two coincide on busy windows and the former is the stricter counterfactual on sparse ones. The null is therefore a \emph{conditional homogeneous-Poisson} process: a homogeneous Poisson process at rate $n_{\mathrm{pkt}}^{\#} / (\max \text{arr}_H - \min \text{arr}_H)$ conditioned on the observed count over the observed support. Per (window, $N$, regime) we record $p_{50}, p_{95}, p_{99}, p_{99.9}, \max$ of end-to-end latency. Aggregation is corpus-median across all windows; the two headline curves per scenario are $p_{99}(N)$ and $p_{50}(N)$ under Hawkes and Poisson overlaid.
...
Table~\ref{tab:main-corpus} reports corpus-median end-to-end latency quantiles for every $(T, N)$ cell of the sweep at $h = 1.7\,\mu\mathrm{s}$, on 276 sessions and 3512 thirty-minute windows (five of the 281 session tapes contain no complete RTH window and are dropped). Figure~\ref{fig:curves} plots the same numbers. The Poisson-null columns are the uniform-shuffle counterfactual of Section~\ref{sec:setup-sweep} with the same packet count per window.
...
\bottomrule
\end{tabular}
\caption{Corpus-median end-to-end latency quantiles in \si{\micro\second} at $h = 1.7\,\mu\mathrm{s}$, for each service floor $T$ (\si{\micro\second}) and stage count $N$. 276 sessions, 3512 thirty-minute RTH windows of CME MDP3 NQ front-month packet arrivals. The Poisson null is the uniform shuffle of the same arrival count per window.}
...
\caption{Corpus-median latency versus stage count $N$ for each service floor $T$ (one shade per $T$, labelled at the right). Left: median. Centre: $p_{99}$, Hawkes solid and Poisson null dashed. Right: the Hawkes tail excess ratio $\Delta(N)/\Delta(1)$ against the Theorem~\ref{thm:reduction} bound $1/N$ (dotted). The median rises by exactly $h$ per hop in both arms; the Hawkes $p_{99}$ falls with $N$ at every $T \ge 4\,\mu\mathrm{s}$ while the Poisson $p_{99}$ tracks the median.}
\label{fig:curves}
\end{figure}

Under the Poisson null the queue is empty when almost every message arrives, so every reported quantile equals the service floor, $p_{50} = p_{99} = T + (N-1)h$, in every cell with $T \le 16\,\mu\mathrm{s}$. The single exception is $T = 32\,\mu\mathrm{s}$, $N = 1$, where Poisson $p_{99}$ is $42.0\,\mu\mathrm{s}$ against a floor of $32$: at the corpus-median packet rate of $452$ packets per second the utilisation there is $\rho = 452 \times 32\,\mu\mathrm{s} = 0.0145$, above the $0.01$ at which Corollary~\ref{cor:poisson} says the Poisson wait must first appear at $p_{99}$, and it does; at $N = 2$ the per-stage utilisation halves to $0.007$ and the wait vanishes again. The $p_{99.9}$ column tells the same story one decade down: the Poisson wait is visible at $N = 1$ for every $T \ge 4\,\mu\mathrm{s}$ (single-stage $\rho > 10^{-3}$) and at $N = 2$ for $T \ge 8\,\mu\mathrm{s}$, and absent wherever the per-stage $\rho/N$ falls below $10^{-3}$. The null arm is not inert; it is a queue with a small, predictable wait, and splitting under Poisson loses $(N-1)h$ at every quantile the operator would look at.

Under Hawkes the story is different at every service floor above the hop cost. Single-stage $p_{99}$ is $2.0$, $2.7$, $3.5$, $5.0$, and $8.0$ times the service floor at $T = 2, 4, 8, 16, 32\,\mu\mathrm{s}$, and single-stage $p_{99.9}$ reaches $15.5\,T$ at $T = 32$. Splitting into two stages cuts the $p_{99}$ from $10.87$ to $7.70\,\mu\mathrm{s}$ at $T = 4$, from $28.2$ to $16.6$ at $T = 8$, from $79.2$ to $37.9$ at $T = 16$, and from $254.7$ to $96.9$ at $T = 32$, in each case for a median cost of exactly $1.7\,\mu\mathrm{s}$; the two-stage $p_{99}$ is below the single-stage $p_{99}$ on $100\%$ of the 3512 windows at every $T \ge 4\,\mu\mathrm{s}$. The stage count that minimises $p_{99}$ rises with $T$: $N = 2$ at $T = 4$, $N = 4$ at $T = 8$ and $16$, $N = 8$ at $T = 32$. At $T = 2\,\mu\mathrm{s}$, below the $2h$ floor, splitting loses on $p_{99}$ ($4.70$ against $4.00$) and wins on only $27\%$ of windows; the row is in the sweep to demonstrate that floor, and it does.

\subsection{Median cost is linear in $N$, exactly as $(N-1)h$}
\label{sec:results-median}

The $p_{50}$ columns of Table~\ref{tab:main-corpus} match the single-message formula of Proposition~\ref{thm:burst} in every one of the twenty cells, under both arms:
\[
p_{50}(N) = T + (N-1)\,h \qquad \text{with } h = 1.7\,\mu\mathrm{s},
\]
for instance $8.00, 9.70, 13.10, 19.90$ at $T = 8$. Under Poisson the identity holds because the queue is empty; under Hawkes it holds because the median message arrives in a quiet inter-cluster stretch where the queue is also empty. The hop tax is therefore exactly $h$ per boundary on the median, with no hidden component, as Theorem~\ref{thm:reduction} says it must be at every quantile.
...
\end{align}
where $\Delta(N) \equiv p_{99}^{\mathrm{Hawkes}}(N) - p_{50}^{\mathrm{Hawkes}}(N)$ is the ``clustering tail excess''. Table~\ref{tab:delta} lists it, together with the ratio $\Delta(N)/\Delta(1)$ that Theorem~\ref{thm:reduction} bounds by $1/N$.
...
\bottomrule
\end{tabular}
\caption{Corpus-median clustering tail excess $\Delta(N) = p_{99}(N) - p_{50}(N)$ under Hawkes arrivals at $h = 1.7\,\mu\mathrm{s}$, its ratio to the single-stage value, and the least-squares exponent $\gamma$ in $\Delta(N) \approx \Delta(1) N^{-\gamma}$. The last row is the Theorem~\ref{thm:reduction} bound.}
\label{tab:delta}
\end{table}

Two features of Table~\ref{tab:delta} are the theorems made visible. First, $\Delta$ depends on $(T, N)$ only through the per-stage service $T/N$: every diagonal of constant $T/N$ carries one number ($1.00$ at $T/N = 1\,\mu\mathrm{s}$, $2.00$ at $2$, $6.87$ at $4$, $20.20$ at $8$), to the reported precision, across up to four different $(T, N)$ pairs. That is the reduction identity Equation~\eqref{eq:reduction} on real arrivals: the tandem's tail is the single-server tail at $s_{\max}$, and the table is really a table of $\Delta_{\mathrm{single}}(s)$ at $s \in \{0.25, 0.5, 1, 2, 4, 8, 16, 32\}\,\mu\mathrm{s}$. Second, every ratio sits at or below the bound $1/N$, and the pathwise check of Section~\ref{sec:evaluation} finds no message on any window above it. At $T = 2\,\mu\mathrm{s}$ the bound is met exactly ($\gamma = 1.00$): the $p_{99}$ message there waits behind exactly one predecessor, so $\Delta(1) = T$ and the tandem can do no better than divide that one service by $N$. As $T$ grows the queues lengthen, idle gaps between clusters drain proportionally more of the tandem's smaller backlog, and $\gamma$ rises to $1.67$ at $T = 32$; that slack is the utilisation effect described after Theorem~\ref{thm:reduction}, and Appendix~\ref{app:check} reproduces it on synthetic Borel clusters.

The operator's stage-count optimum is
\[
N^\star \;=\; \arg\min_N \Big[ (N-1) h \;+\; \kappa \, \Delta(N) \Big]
\]
for the operator's tail weight $\kappa$ (the number of microseconds of median the operator will spend to remove one microsecond of $p_{99}$ excess). Over the simulated grid $N \in \{1, 2, 4, 8\}$ at $h = 1.7\,\mu\mathrm{s}$ the breakpoints are:
\begin{center}
...
\end{tabular}
\end{center}
An operator who values tail and median equally ($\kappa = 1$) lands on $N^\star = 1$ at $T = 2$, $2$ at $T = 4$, $4$ at $T = 8$, $4$ at $T = 16$, and $8$ at $T = 32\,\mu\mathrm{s}$, which is also the ordering of the raw $p_{99}$ minima in Table~\ref{tab:main-corpus}.
...
\]
which Table~\ref{tab:delta} lets the operator evaluate directly, since its diagonals are $\Delta_{\mathrm{single}}(s)$ on a power-of-two grid of $s$. Two rules of thumb fall out. A cut is worth its hop only if it lowers $s_{\max}$ by enough that $\kappa\,[\Delta_{\mathrm{single}}(s_{\max}) - \Delta_{\mathrm{single}}(s_{\max}')] > h$; and among partitions with the same $s_{\max}$ the one with the fewest hops wins outright, so sub-tasks downstream of the bottleneck should be merged onto one thread until their sum reaches $s_{\max}$.

\begin{center}
\fbox{\begin{minipage}{0.92\linewidth}
\textbf{Design principle (only $s_{\max}$ matters).} For a servicing chain cut at fixed boundaries, the tail-latency question is not ``how many threads?'' but ``what is the smallest achievable largest stage $s_{\max}$?'' By Theorem~\ref{thm:reduction} the clustering tail excess of any partition is $\Delta_{\mathrm{single}}(s_{\max})$, independent of the stage count; a cut that does not lower $s_{\max}$ compresses no tail and still costs one hop $h$. Choose the partition with the smallest reachable $s_{\max}$, then the fewest hops that realise it.
\end{minipage}}
\end{center}

\paragraph{Floor on $T$: short tasks cannot be usefully split.} Equations~\eqref{eq:design99}--\eqref{eq:design50} carry an obvious lower bound on the tasks the paper's recommendation applies to. Splitting a task of total service $T$ into $N$ stages spends $(N-1) h$ at every quantile to compress a tail excess $\Delta(1)$ that is itself a small multiple of $T$ ($2$ to $7\,T$ on this corpus, Table~\ref{tab:delta}). When $T$ is of order the hop cost $h$, one hop already costs as much as the entire tail excess it could remove, and the design equation prescribes $N^\star = 1$ mechanically. The sweep shows exactly this: at $T = 2\,\mu\mathrm{s}$, $\Delta(1) = 2.00\,\mu\mathrm{s}$ and the best a split can do is remove half of it ($1.00\,\mu\mathrm{s}$) for a cost of $1.7\,\mu\mathrm{s}$, so $p_{99}$ rises from $4.00$ to $4.70$; at $T = 4\,\mu\mathrm{s}$, $\Delta(1) = 6.87\,\mu\mathrm{s}$, the split removes $4.87$ of it for the same $1.7$, and $p_{99}$ falls from $10.87$ to $7.70$ on every window. On this framework's shipping BQueue path ($h \approx 1.7\,\mu\mathrm{s}$) the floor is therefore $T \gtrsim 2h \approx 3.4\,\mu\mathrm{s}$; $T = 4\,\mu\mathrm{s}$ sits just above it and already wins. Sub-microsecond tasks, for instance a single SBE integer decode or a small handler that fires from a strategy fast path, belong on \texttt{fast\_send} rather than on a cross-thread mailbox, regardless of feed statistics.

\subsection{Across windows}
\label{sec:results-windows}

The corpus medians understate the tail on the busiest windows. Across the 3512 windows the single-stage $p_{99}/T$ has a median of $2.0, 2.7, 3.5, 5.0, 8.0$ and a 95th percentile of $2.8, 3.3, 4.4, 6.2, 10.7$ at $T = 2, 4, 8, 16, 32\,\mu\mathrm{s}$, so one window in twenty at $T = 32$ has a single-stage $p_{99}$ above $340\,\mu\mathrm{s}$. The tandem's win is not confined to the median window: $p_{99}(2) < p_{99}(1)$ holds on $100\%$ of windows for every $T \ge 4$, and $p_{99}(4) < p_{99}(1)$ on $74.5\%$ of windows at $T = 4$ and $100\%$ at every $T \ge 8$. The per-window fitted branching ratio has median $0.794$ and a 5th--95th percentile range of $0.757$--$0.827$ (Section~\ref{sec:setup-hawkes}); the tail excess tracks it in the expected direction, with the high-$n$ windows supplying the upper tail of the $p_{99}/T$ distribution. The intervening $p_{95}$ quantile is recorded per window in the sweep (Section~\ref{sec:setup-sweep}) and omitted from the tables above only for brevity; it lies between the reported $p_{50}$ and $p_{99}$ in every cell by construction, and under the Poisson null it equals the service floor wherever $p_{99}$ does, since $\rho/N < 1 - 0.99$ implies $\rho/N < 1 - 0.95$. The 3512 windows are treated as evaluation units for the descriptive statistics reported here; they are nested within 276 sessions and are not independent, so any statistical uncertainty on the aggregates would be clustered at the session level rather than treating 3512 as an independent sample size.

\subsection{Sensitivity to $h$}
\label{sec:results-h}

The design equation is monotone in $h$: at fixed $T$ and fixed corpus $\Delta(N)$, every $\kappa$ breakpoint in the table above scales linearly with $h$, so halving $h$ halves every breakpoint and moves $N^\star$ up one notch at every $T$ for a fixed $\kappa$. At $h = 1.7\,\mu\mathrm{s}$ the $p_{99}$-minimising $N$ is $1, 2, 4, 4, 8$ across the five service floors; at a hypothetical sub-microsecond busy-poll mailbox it would be $2, 4, 8, 8, 8$ and the natural cut-point ceiling of $4$--$5$ stages would bind before the design equation did. For any other framework the procedure is the same: measure the one-way asynchronous hop cost $h$ on its own dispatch path, put it into Equation~\eqref{eq:design99} with the corpus $\Delta$ of Table~\ref{tab:delta}, and read off $N^\star$; a framework whose $h$ exceeds the tail excess a split could remove gets $N^\star = 1$, and the recommendation reverses.
...
\paragraph{Proposition~\ref{thm:burst} (burst-limit throughput).} The single-message identity $p_{50}(N) = T + (N-1) h$ is predicted exactly by Proposition~\ref{thm:burst} for any arrival law when the median message finds the queue empty. Table~\ref{tab:main-corpus} confirms it in all twenty $(T, N)$ cells under both arms at $h = 1.7\,\mu\mathrm{s}$, to the $0.01\,\mu\mathrm{s}$ reporting precision.

\paragraph{Theorem~\ref{thm:reduction} (reduction and scaling bound).} The prediction is $\Delta(N) \le \Delta(1)/N$ on every window, and more strongly $W_j^{(N)} \le W_j^{(1)}/N + (N-1)h$ for every individual message. The corpus-median ratios in Table~\ref{tab:delta} sit at or below $1/N$ in every cell, with equality at $T = 2\,\mu\mathrm{s}$ and slack growing with $T$ ($\gamma$ from $1.00$ to $1.67$). The pathwise form was checked directly by the simulator: across all 3512 windows, all five service floors, both arms, and $N \in \{2, 4, 8\}$, no message violates the bound (the check allows $N$ nanoseconds for the rounding of $T/N$; the largest excess over that tolerance is $0$). The reduction identity Equation~\eqref{eq:reduction} is visible in Table~\ref{tab:delta} without any fitting: $\Delta$ is constant along every diagonal of constant $T/N$, so the tandem's tail is the single-server tail at $s_{\max}$ on real arrivals exactly as on synthetic ones (Appendices~\ref{app:check} and~\ref{app:unequal}).

\paragraph{Corollary~\ref{cor:poisson} (Poisson null).} The prediction is $p_{q}^{\mathrm{P}}(N) = T + (N-1)h$ wherever the per-stage utilisation satisfies $\rho / N < 1 - q$, and a visible wait wherever it does not. Table~\ref{tab:main-corpus} matches both halves: Poisson $p_{99}$ equals the floor in nineteen of twenty cells and exceeds it only at $T = 32\,\mu\mathrm{s}$, $N = 1$ ($\rho = 0.0145 > 0.01$), and Poisson $p_{99.9}$ exceeds the floor exactly in the cells with $\rho/N > 10^{-3}$ (single stage at $T \ge 4$, two stages at $T \ge 8$, four stages at $T \ge 16$) and nowhere else. The null arm is therefore a working queue whose small wait appears where PASTA says it must, and splitting under it costs $(N-1)h$ at every reported quantile.

\paragraph{Verdict.} All three analytical statements survive the full-corpus test. The quantitatively load-bearing one, the pathwise bound of Theorem~\ref{thm:reduction}, holds on every message of every window, and its slack is measured rather than assumed: $\gamma = 1.00, 1.23, 1.48, 1.65, 1.67$ at $T = 2, 4, 8, 16, 32\,\mu\mathrm{s}$.
...
The recommendations in this Part are stated for HFT systems in general, but they are worked out on one concrete system, and the reader should know what that system is. \kasparhft{} \citep{kaspar_repo,mayeski_fastsend,mayeski_actorhft} is an open-source C++20 trading system for CME futures built on a shared-nothing actor framework, available at \url{https://github.com/vincent212/kaspar-hft}: every component (socket reader, feed arbitrator, SBE decoder, order book, strategy, order manager, iLink session) is an actor with private state and a mailbox, and components interact only by passing messages. It is at once a production system (MDP3 multicast in, full order books reconstructed order by order, the \shadowpov{} execution algorithm \citep{maciejewski_shadowpov} on top, authenticated iLink~3 sessions out) and, with the same actor code driven from recorded packet captures, a deterministic replay simulator in its own right. Part~II does not run on that simulator: its measurements are made on a standalone tandem-Lindley model of the pipeline (Section~\ref{sec:setup-simulator}), driven by packet-arrival timestamps extracted from the same class of capture. The framework supplies the architecture under study and the calibration of $h$; it is not in the measurement loop. The \texttt{fast\_send} paper documents the four extensions that make the actor model usable at HFT latencies: \texttt{fast\_send} itself, a synchronous delivery in which the sending thread runs the receiver's handler inline and takes the reply as a value; actor \texttt{Group}s, which co-schedule a set of actors on one thread behind a single mailbox; mailbox queue implementations selectable per actor; and a memory pool that removes per-message allocation and its tail. The defining property of \texttt{fast\_send} is \emph{receiver transparency}: a handler is written once and has no way to tell whether it was invoked synchronously or asynchronously, on which thread, or whether its sender is blocked. That property is what Section~\ref{sec:implementation} builds on.

The reference pipeline of that paper is the design context this paper revisits. It has three sections joined by exactly two cross-thread hops. Two socket-reader actors, one per redundant multicast channel, do nothing but drain their UDP sockets and hand datagrams to a buffer actor by an asynchronous \texttt{send}; that hop is asynchronous because a reader that blocked on its callee would drop packets under a burst. From the buffer, arbitration between the A and B feeds, SBE decode, order-book construction, and signal generation run as a single chain of \texttt{fast\_send}s on one thread, with no queue and no context switch between stages. When the signal decides to act it hands the order to an order-management actor by a second asynchronous \texttt{send}, because the iLink write takes microseconds and blocking the decode thread on it would stall the market-data queue. The measured costs behind that layout are Table~4 of the \texttt{fast\_send} paper: a cross-thread \texttt{send} round trip of $\sim 3.4\,\mu\mathrm{s}$ (a mailbox wakeup across cores), a same-thread grouped \texttt{send} of $90$~ns, and a \texttt{fast\_send} of $\sim 30$~ns. With a servicing chain of $\sim 7\,\mu\mathrm{s}$ measured tick-to-book on live ES, NQ, and ZN feeds, the framework's own per-message contribution is under $1\%$ of the floor, and the same measurement already found that the latency tail was set by the arrival process, which was strongly clustered, with a Fano-implied branching ratio of $0.85$--$0.97$. That observation is the seed of the present paper.

Read against Part~II, the reference pipeline is the single-thread servicing chain of Section~\ref{sec:intro-singlethread-priors} realised with actors: the whole $T$ sits on one thread between two hops that were accepted because the alternative, blocking the chain, was worse. The two existing hops show that the design was never opposed to asynchronous handoffs as such; it placed them where the cost of not having them was obvious. What this paper adds is that the same logic applies inside the chain. On a Hawkes-clustered feed the $\sim 7\,\mu\mathrm{s}$ servicing chain sits above the $2h \approx 3.4\,\mu\mathrm{s}$ floor of Section~\ref{sec:results-design}, its single-thread $p_{99}$ carries a tail excess of several $T$ (Table~\ref{tab:delta}), and cutting it at its natural boundaries into stages joined by asynchronous \texttt{send} compresses that excess by the factor of Theorem~\ref{thm:reduction} for a median cost of one hop per cut. The recommendations that follow are therefore stated as changes to this particular pipeline: where to cut it, in what order to place the stages, which boundaries to leave on \texttt{fast\_send}, and how to decide each of those by measurement rather than by reasoning at a single call site.

\paragraph{The rule applied to this pipeline.} The design equation can be run on the reference chain directly, using Table~\ref{tab:delta} as a lookup of the single-server tail excess $\Delta_{\mathrm{single}}(s)$ against per-stage service $s$. Take the measured $\sim 7\,\mu\mathrm{s}$ servicing chain and the four natural boundaries inside it (arbitration, SBE decode, book application, signal), and suppose for the sake of the arithmetic that the decode and book stages dominate at roughly $3\,\mu\mathrm{s}$ each with arbitration and signal near $0.5\,\mu\mathrm{s}$. The partitions available at those cut points, with $h = 1.7\,\mu\mathrm{s}$ and $\Delta_{\mathrm{single}}$ interpolated on the power-of-two grid of Table~\ref{tab:delta}, are:
...
Three things follow, and each is an instance of a general rule rather than a fact about this pipeline. The balanced two-way cut is the best of the four: it more than halves the tail excess for one hop. Going to three stages lowers $s_{\max}$ only from $3.5$ to $3.0\,\mu\mathrm{s}$, because the book stage is already the bottleneck and the third cut merely separates two sub-tasks that were not binding; the $1.3\,\mu\mathrm{s}$ it buys does not cover the $1.7\,\mu\mathrm{s}$ hop it costs. Splitting all four is strictly worse again: the fourth hop lowers $s_{\max}$ not at all, which is Remark~\ref{rem:unequal}'s third consequence in concrete form. The \shadowpov{} execution algorithm \citep{maciejewski_shadowpov} rides the signal stage of this chain; at the illustrative $\sim 0.5\,\mu\mathrm{s}$ it is the smallest of the four sub-tasks and sits well below the book-application bottleneck, so the design equation keeps it on \texttt{fast\_send} with its neighbour rather than on a mailbox hop of its own (Remark~\ref{rem:unequal}, downstream-of-bottleneck merge) and no cut falls on it. So the recommendation for this pipeline is one cut, at the decode/book boundary, with the book stage placed at ingress if the decode is the smaller of the two --- and the numbers above are a prior for what the end-of-build measurement of Section~\ref{sec:fungibility-workflow} should find, not a substitute for running it. The per-stage service times are illustrative; the operator's own profile of the chain replaces them, and the lookup table does the rest.
...
\paragraph{Two hop paths in the shipping \kasparhft{} framework.} Of the delivery mechanisms described in Section~\ref{sec:kaspar}, two matter for the design equation. The default asynchronous \texttt{send} uses \texttt{BQueue}, a blocking mailbox built on \texttt{std::mutex} $+$ \texttt{std::condition\_variable} $+$ \texttt{boost::circular\_buffer}; a producer thread enqueues under the mutex and a consumer thread waits on the condition variable. This is the hop path the paper's Section~\ref{sec:setup-simulator} model corresponds to, and halving its measured $\sim 3.4\,\mu\mathrm{s}$ round trip gives the one-way hop cost $h \approx 1.7\,\mu\mathrm{s}$ used throughout Part~II.

The \texttt{fast\_send} path is different: it is a \emph{synchronous} dispatch that acquires a per-actor mutex, calls the receiver's handler inline on the sender's thread, and returns the reply. There is no queue and no cross-thread wake-up. Its per-call cost is much smaller than the BQueue path's, but it is not a pipelined hop: the sender blocks until the receiver finishes, so a chain of \texttt{fast\_send}s serialises through all stages on one thread and offers none of the throughput speedup of Proposition~\ref{thm:burst}. The paper's design equation does not apply to \texttt{fast\_send} chains.
...
\begin{enumerate}
  \item Draft the pipeline as a chain of actors at the natural cut points of the servicing chain (\texttt{SocketReader}, SBE decode, order book, strategy, order manager on CME MDP3; four to five stages, with no artificial subdivision of any one of them).
...
\paragraph{Conclusion 1: the thread-hop cost is a wash under real arrivals.} The dominant term in end-to-end latency on a Hawkes-clustered feed is not the mailbox handoff --- it is the cluster-wait tail. On 3512 windows at $h = 1.7\,\mu\mathrm{s}$ the corpus-median single-thread $p_{99}$ is $2.7$, $3.5$, $5.0$, and $8.0$ times the service floor at $T = 4, 8, 16, 32\,\mu\mathrm{s}$, i.e.\ a tail excess of $4$ to $130$ times the hop cost; the $p_{99.9}$ excess is twice that again. Adding a hop is $+1.7\,\mu\mathrm{s}$ at every quantile; the tail it lets us compress is one to two orders of magnitude larger whenever $T$ sits above the $2h$ floor. The single-thread objection --- ``every hop is a fixed tax that no benefit can overcome'' --- is empirically wrong on this class of feed.

\paragraph{Conclusion 2: two threads beat one on the tail at the cost of one hop on the median.} Splitting a service task into two half-length actors on separate threads joined by one $1.7\,\mu\mathrm{s}$ mailbox hop lowers $p_{99}$ on $100\%$ of the 3512 windows at every service floor from $4$ to $32\,\mu\mathrm{s}$, cutting the corpus-median tail excess to between $0.28$ and $0.34$ of its single-thread value for a median cost of exactly $h$; the trade favours the split for any tail weight $\kappa \gtrsim 0.35$ at $T = 4\,\mu\mathrm{s}$ and $\kappa \gtrsim 0.01$ at $T = 32\,\mu\mathrm{s}$. At $T = 2\,\mu\mathrm{s}$, below the $2h$ floor of Section~\ref{sec:results-design}, the split loses on $p_{99}$, as the design equation says it must. The pattern extends to $N > 2$: an $N$-stage tandem contracts the tail excess by a factor of $N$ or better on every sample path (Theorem~\ref{thm:reduction}) while paying $(N-1)h$ at every quantile, and the $p_{99}$-minimising stage count rises with the task length, from $2$ at $4\,\mu\mathrm{s}$ to $8$ at $32\,\mu\mathrm{s}$, up to the number of natural cut points in the decode chain (four or five on CME). The single-thread event loop is dominated on the tail on Hawkes-clustered feeds above the $2h$ floor, and holds its advantage only on the median, by one hop per stage.
...
All code referenced below lives in the \kasparhft{} repository \citep{kaspar_repo}, \url{https://github.com/vincent212/kaspar-hft}, which also carries the actor framework the paper's recommendations are stated for.
...
\bibitem[Maciejewski(preparation)]{mayeski_actorhft}
Maciejewski, V. (in preparation). \kasparhft: a shared-nothing actor framework for CME futures --- fast\_send hop-cost measurement.

\bibitem[Maciejewski(2025)]{mayeski_fastsend}
Maciejewski, V. (2025). \texttt{fast\_send}: synchronous inline dispatch on the CME MDP3 hot path. M2 Tech technical report, \kasparhft{} repository. \url{https://github.com/vincent212/kaspar-hft/blob/main/tech_reports/fast_send.pdf}

\bibitem[Maciejewski(2026b)]{kaspar_repo}
Maciejewski, V. (2026). \kasparhft: an open-source C++20 actor framework and trading system for CME futures. Source repository. \url{https://github.com/vincent212/kaspar-hft}

\bibitem[Maciejewski(2026)]{maciejewski_shadowpov}
Maciejewski, V. (2026). Model-free passive execution via order-level shadowing. arXiv:2609.18019. \url{https://arxiv.org/abs/2609.18019}
