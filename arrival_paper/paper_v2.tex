...
The $p_{50}$ columns of Table~\ref{tab:main-corpus} match the single-message formula of Theorem~\ref{thm:burst} in every one of the twenty cells, under both arms:
\[
...
\paragraph{Theorem~\ref{thm:burst} (burst-limit throughput).} The single-message identity $p_{50}(N) = T + (N-1) h$ is predicted exactly by Theorem~\ref{thm:burst} for any arrival law when the median message finds the queue empty. Table~\ref{tab:main-corpus} confirms it in all twenty $(T, N)$ cells under both arms at $h = 1.7\,\mu\mathrm{s}$, to the $0.01\,\mu\mathrm{s}$ reporting precision.
